%[text] # 确定信号模型下的单源 DoA 集中似然估计（网格搜索）
%[text] 确定信号（deterministic）信号模型下，DoA 估计问题可以表示为如下优化问题
%[text]{"align":"center"} $\\underset{\\mathbf{\\theta}, \\mathbf{x}\_t, \\sigma}{\\mathrm{minimize}}\\quad MT \\ln \\sigma + \\frac{1}{\\sigma} \\sum\_{t=0}^{T-1} \\left\\| \\mathbf{y}\_t - \\mathbf{Ax}\_t \\right\\|^2, \\\\\n\\text{subject to}\\quad \\theta\_k \\in\[-\\pi/2,\\pi/2\] \\\\\n\\qquad\\qquad\\,\\,\\,\\,\\sigma \\geq 0,$
%[text] 其中$\\mathbf{x}\_t$和$\\mathbf{y}\_t$分别是$t$时刻的信源发送信号和阵列接收信号，$T$为时刻/快拍数，$M$为阵元数，$\\mathbf{\\theta}$是$K$个信号源的和来向，$\\sigma\n$是噪声功率。为了降低问题求解的维度，将$\\mathbf{x}\_t$和$\\sigma\n$的估计分别用$\\mathbf{\\theta}$表示为
%[text]{"align":"center"} $\\hat{\\mathbf{x}}\_t = \\left\[ \\mathbf{A}^\\mathrm{H} \\mathbf{A} \\right\]^{-1} \\mathbf{A}^\\mathrm{H} \\mathbf{y}\_t,$
%[text]{"align":"center"} $\\hat{\\sigma}(\\mathbf{\\theta}) = \\frac{1}{M} \\mathrm{tr} (\\mathbf{I} - \\mathbf{A}(\\mathbf{A}^\\mathrm{H}\\mathbf{A})^{-1}\\mathbf{A})\\hat\\mathbf{R} = \\frac{1}{M} \\mathrm{tr}\[(\\mathbf{I} - \\mathcal{P}\_\\mathbf{A} \\hat\\mathbf{R}\],$
%[text] 其中$\\hat\\mathbf{R} = \\frac{1}{T} \\sum\_{t=0}^T \\mathbf{y}\_t \\mathbf{y}\_t^\\mathrm{H}$，则可以得到$\\mathbf{\\theta}$的集中似然估计问题
%[text]{"align":"center"} $\\mathbf{\\theta}\_\\mathrm{CDML} = \\arg\\min\_{\\mathbf{\\theta}} \\frac{1}{M} \\mathrm{tr} \[(\\mathbf{I} - \\mathcal{P}\_\\mathbf{A}) \\hat\\mathbf{R}\].$
...
function [sp, x, p, noise_var] = mle_det_con_grid_1d(y, k, design, wavelength, grid_size, varargin)
unit = 'radian';
...
% Deterministic ML estimate of source snapshots (k x T)
x = Ginv * (A' * y);

% Estimate signal covariance matrix P and noise variance
noise_var = real(trace((eye(M) - A * Ginv * A') * R));

% Source power: time-averaged |X_hat|^2
p = real(mean(abs(x).^2, 2));
%[text] ### store results
sp = struct();
...
for ii = 1:n_theta
  ...
  P_A = A * Ginv * A';          % Projection matrix onto Col(A)
  cost = trace((eye(M) - P_A) * R);  % tr((I - P_A) R_hat)

  v(ii) = -real(cost);          % larger v = better DoA
end
end
