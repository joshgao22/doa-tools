...
%[text] ## Syntax
%[text] `sp = mle_sto_con_grid_2d(R, k, design, wavelength, grid_size, ...)`
%[text] `sp = mle_sto_con_grid_2d(R, k, f_steering, [], grid_size, ...)`
%[text] `[sp, p, noise_var] = mle_sto_con_grid_2d(R, k, design, wavelength, grid_size, ...)`
%[text] `[sp, p, noise_var] = mle_sto_con_grid_2d(R, k, f_steering, [], grid_size, ...)`
%[text] ## Inputs
...
function [sp, p, noise_var] = mle_sto_con_grid_2d(R, k, design, wavelength, grid_size, varargin)
unit = 'radian';
...
if ishandle(design)
  A_all = design(wavelength, theta);
else
  A_all = steering_matrix(design, wavelength, theta);
end
