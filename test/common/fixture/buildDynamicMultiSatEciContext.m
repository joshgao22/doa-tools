function context = buildDynamicMultiSatEciContext(varargin)
%BUILDDYNAMICMULTISATECICONTEXT Build one reusable multi-satellite dynamic context.
% This helper centralizes the paper-facing multi-satellite scene / waveform
% setup while keeping estimator strategy and replay logic at the caller.

opt = localParseContextOpt(varargin{:});

if opt.numUsr ~= 1
  error('buildDynamicMultiSatEciContext:OnlySingleUserSupported', ...
    'The current dynamic context only supports one user.');
end
if ~any(opt.masterOffsetIdx == 0) || ~any(opt.periodicOffsetIdx == 0)
  error('buildDynamicMultiSatEciContext:MissingReferenceFrame', ...
...
if numel(unique(selectedSatIdxGlobal, 'stable')) ~= numel(selectedSatIdxGlobal)
  error('buildDynamicMultiSatEciContext:DuplicateSatIdx', ...
    'selectedSatIdxGlobal must not contain duplicate satellite indices.');
end
...
if ~any(selectedSatIdxGlobal == refSatIdxGlobal)
  error('buildDynamicMultiSatEciContext:InvalidReferenceSat', ...
...
nonRefSatIdxLocal = setdiff(1:sceneSeqMaster.numSat, refSatIdxLocal, 'stable');
nonRefSatIdxGlobal = selectedSatIdxGlobal(nonRefSatIdxLocal);
...
context.satPickCount = numel(selectedSatIdxGlobal);
...
context.nonRefSatIdxLocal = reshape(nonRefSatIdxLocal, 1, []);
context.nonRefSatIdxGlobal = reshape(nonRefSatIdxGlobal, 1, []);
context.otherSatIdxLocal = [];
context.otherSatIdxGlobal = [];
if sceneSeqMaster.numSat == 2
  context.otherSatIdxLocal = context.nonRefSatIdxLocal;
  context.otherSatIdxGlobal = context.nonRefSatIdxGlobal;
end
...
opt.baseSeed = 253;
opt.usrLla = [55; 36.59; 0];
opt.utcRef = datetime([2026, 03, 18, 17, 08, 0], ...
  'TimeZone', 'UTC', ...
  'Format', 'yyyy-MM-dd HH:mm:ss.SSSSSS');
opt.tleFileName = "statlink_20260318.tle";
...
elseif mod(nargin, 2) == 0
  for iArg = 1:2:nargin
    ...
    if ~(ischar(name) || (isstring(name) && isscalar(name)))
      error('buildDynamicMultiSatEciContext:InvalidNameValue', ...
        'Name-value inputs must use character or string names.');
    end
    opt.(char(name)) = varargin{iArg + 1};
  end
elseif nargin ~= 0
  error('buildDynamicMultiSatEciContext:InvalidInput', ...
...
elseif ischar(utcInput) || (isstring(utcInput) && isscalar(utcInput))
  ...
  error('buildDynamicMultiSatEciContext:InvalidUtcRef', ...
