...
multiStartDiag.selectedStartTag = "single";
multiStartDiag.startTagList = strings(0, 1);
...
startTagList = strings(numCandidate, 1);
...
for iCand = 1:numCandidate
  ...
  initParamCur = [];
  startTagList(iCand) = sprintf("cand%d", iCand);
...
selectedIdx = localSelectBestDynamicCandidate(fvalList, isResolvedList, startTagList, iterationList, objImproveList, moveNormList);
estResultBest = resultCell{selectedIdx};

multiStartDiag.isEnabled = true;
multiStartDiag.selectedIdx = selectedIdx;
multiStartDiag.selectedStartTag = startTagList(selectedIdx);
multiStartDiag.startTagList = startTagList;
...
function selectedIdx = localSelectBestDynamicCandidate(fvalList, isResolvedList, startTagList, iterationList, objImproveList, moveNormList)
%LOCALSELECTBESTDYNAMICCANDIDATE Select the strict best-fval candidate.
% Use final objective value as the primary selection key. Only use release-
% oriented metrics as tie-breaks when candidates are numerically tied.
...
objTol = max([1e-3, 1e-9 * max(abs(bestFval), 1)]);
...
preferStatic = contains(lower(string(startTagList(nearBest))), "static");
