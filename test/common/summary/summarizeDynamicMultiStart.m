...
for iCase = 1:numel(caseList)
  ...
  startTagList = reshape(string(localGetFieldOrDefault(multiStart, 'startTagList', strings(0, 1))), [], 1);
  numRow = numel(startTagList);
  if numRow == 0
    continue;
  end

  selectedIdx = localGetFieldOrDefault(multiStart, 'selectedIdx', NaN);
  selectedStartTag = string(localGetFieldOrDefault(multiStart, 'selectedStartTag', ""));
  ...
  bestIdx = NaN;
  bestFval = NaN;
  validFval = find(isfinite(fvalList));
  if ~isempty(validFval)
    [bestFval, idxRel] = min(fvalList(validFval));
    bestIdx = validFval(idxRel);
  end
  ...
  for iRow = 1:numRow
    ...
    rowInfo.startTag = startTagList(iRow);
    rowInfo.isSelected = iRow == selectedIdx;
    rowInfo.isBestFval = iRow == bestIdx;
    rowInfo.selectedStartTagMatch = startTagList(iRow) == selectedStartTag;
    rowInfo.bestFvalStartTagMatch = iRow == bestIdx;
