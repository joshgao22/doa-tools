...
- 作用：专门排查 `scanSfStaticMleCrbConsistency.m` 中 `MS-SF-DoA` 的 MLE/CRB gap，目标是区分 CRB / metric / FIM 口径问题与 DoA-only MLE solver / local-basin 问题。当前该入口已经完成 `MS-SF-DoA` CRB-scale audit，不再作为继续扩展 probe 的主入口。
- 职责边界：不替代 `scanSfStaticMleCrbConsistency.m` 的正文统计曲线，也不修改 static / dynamic estimator 默认路径；它只做小 MC 与少量 probe，用于回答“CRB 是否同口径”和“MS DoA-only MLE 是否吃到多星增益”。replay-local amp-aware CRB 只用于诊断，不等同于正式 paper-facing CRB helper。
- 诊断口径：输出 baseline `SS-SF-DoA`、`Other-SF-DoA`、baseline `MS-SF-DoA`，并用 compact `truthInitDiffTable` 确认 truth-init 与 baseline 是否有差异；DoA-only truth-init 只使用 truth 作为初值，仍走完整 DoA 搜索域。CRB 同时报告 single / other / joint 的 single-frame DoA-Doppler CRB、deterministic DoA-only unit-gain CRB，以及 replay-local pilot-model amp-aware CRB；配套保存 expected-vs-realized DoA gain、empirical joint-gain prediction、FIM additivity、truth/final objective probe、per-sat mean-noisy numerical Hessian aggregate、noise-free sanity preview 和 top-tail 预览。
- 存储与工程壳：默认启用 checkpoint / resume、progressbar、snapshot 与 Telegram 通知；任务文件写入仓库根目录 `tmp/replaySfMsDoaCrbDiagnose/<runKey>/task/`，成功构造 `replayData` 后清理 checkpoint。summary / plot / Telegram 报告只从 `replayData` 读取，恢复 snapshot 后可直接运行 summary section。
- 当前结论口径：`MS-SF-DoA` 的 unit-CRB gap 已定位为 DoA-only CRB signal-scale 口径差，而不是 estimator / truth-init / fusion failure。第二星 effective pilot gain 约 `0.348`，其有效 FIM 约为 unit-gain 假设下 `12%`；使用 replay-local amp-aware CRB 后，`Other-SF-DoA` 与 `MS-SF-DoA` 均回到 CRB-level 附近。后续不继续调 `MS-SF-DoA` estimator，也不继续向本 replay 添加 probe；下一步应 formalize deterministic CRB 的 per-sat amplitude / effective-SNR scaling，再回到 `scanSfStaticMleCrbConsistency` 验证四种 static case。
- 详细结果：`results/replaySfMsDoaCrbDiagnose.md`。
