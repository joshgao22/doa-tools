...
- 默认扫描：文件头默认按 `snrDbList=(-15:5:10).' × seedList` 构造 task grid；当前代表性结果固定为 `numRepeat=200`、seed `253:452`、`P=10`、DoA hard box `2×CRB`、MF `fdRef` truth-local `0.300 tooth` half-width。快速 smoke test 时可临时把 `numRepeat` 降到 10 或 20。该脚本已经在 aggregate、compare、search-range audit 与 CRB path audit 中按 SNR 分组；后续更系统的 DoA box scale、fdRef half-width、geometry、frame count 与 information-loss 曲线交给 scan，不再在 CleanTrim 内继续扩展扫描维度。
- 固定路线：默认跑 `SS-SF-Static / MS-SF-Static / SS-MF-CP-K / SS-MF-CP-U / MS-MF-CP-K / MS-MF-CP-U`。`CP-K` 是 known-rate baseline，`CP-U` 是 unknown-rate nuisance 口径；四条 MF 路线均保持 `pure-mle / core-only`，不再默认跑 `SS-MF-CP-U-Robust`。SS 与 MS 的主链路仍是：先在同一个 truth-centered `fdRef` 搜索盒和按方法 CRB 缩放的 DoA hard bound 内运行对应 SF static estimator，再用各自 static 结果作为 MF solver seed；MS-MF 只读取 MS-SF-Static 结果，不读取 SS-MF final。
- MF 口径：文件头 `dynamicObjectiveMode="pure-mle"` 与 `dynamicRouteMode="core-only"` 是当前默认，用于检查最干净的 one-shot MLE core：关闭 CP engineering penalty、fd alias unwrap、DoA basin-entry、known embedded branch 与 unknown warm-anchor。MF 的 DoA solver seed 可以来自 static 结果，但 DoA hard bound 固定为 truth-centered `truthLocalDoaCrbScale × 当前方法 CRB std`，不随 static seed 漂移，也不使用固定绝对角度宽度；若 static seed 超出该 hard bound，由 estimator init check 裁回 box 内。此前用于 SS route 对照的 robust 行已从默认 method list 删除；如需复查 basin-entry，只在临时诊断中显式改 route，不混入 CleanTrim 主输出。
...
- 主要输出：`caseTable`、`cleanAggregateTable`、`cleanHealthAggregateTable`、`cleanTrimAggregateTable`、`cleanSsMsCompareTable`、`cleanKnownUnknownCompareTable`、`cleanSnrCurveTable`、`cleanEstimateCrbCurveTable`、`cleanSearchRangeAuditTable`、`cleanCrbPathAuditTable`、`cleanCrbPathAuditAggregateTable`、`cleanOutlierTable`、`runtimeTable / runtimeAggregateTable / topSlowRuntimeTable`。其中 `cleanSsMsCompareTable` 同时报告 MS/SS 绝对 angle RMSE、MS 相对自身 CRB 的 MSE、MS 相对 SS-MF CRB 的 MSE、fdRef MSE/CRB、keep rate 与 `targetClass`，用于判断 MS-MF 是已达 CRB-level、仅在绝对 RMSE 上优于 SS，还是仍然 not-yet；`cleanKnownUnknownCompareTable` 在 raw / health / joint-trim 口径下分别比较 SS 与 MS 的 `CP-K` / `CP-U` angle RMSE、angle MSE/CRB、fdRef MSE/CRB、keep rate 和 K/U class，用于判断 known-rate baseline 是否按预期优于或接近 unknown-rate 口径；`cleanCrbPathAuditAggregateTable` 汇总 K/U monotonicity、EFIM Schur loss PSD、shared block、reference `fdRef/fdRate` invariant、`gamma_ref` coupling、angle/fdRef CRB U/K ratio 与 `primaryAuditClass`，用于判断 K/U CRB 接近是合理 FIM 投影还是路径错误；`cleanEstimateCrbCurveTable` 优先使用 joint-trim aggregate，若无 trim 则回退 health / raw，用于脚本末尾的对数坐标图。该图在同一 figure 中用两个子图分别显示 `SS/MS-SF-Static` 与 `SS/MS-MF-CP-K/U` 的 spherical angle RMSE 及对应 CRB std、`fdRef` RMSE 及对应 CRB std，y 轴强制采用 log scale，便于直接观察 static/MF、known/unknown 与 SS/MS 的绝对误差和下界随 SNR 的变化。
...
- 当前定位：该 replay 已固定为 representative / diagnostic-only / fixed-bound local sanity 入口。`replayMfMsMleCrbCleanTrim_20260513-181333.mat` 显示 SS-MF 在 bounded local 口径下稳定贴近 CRB，MS-MF 在 `2×CRB` DoA hard box 内形成绝对 angle RMSE gain，并在部分 SNR 达到 own-CRB level；但 `3×CRB` sensitivity 已显示 MS 会明显远离 own-CRB，因此 `2×CRB` 结果必须解释为强 oracle local guard，而不是自然 basin-entry 能力。K/U CRB path audit 为 `path-ok`，说明 K/U angle CRB 接近不应再作为 CRB 主公式错误处理；unknown-rate information loss 后续应通过 Schur loss、fdRef inflation、主方向 loss 与专门 scan 展示。当 `replayMfMsMleCrbFlowDiagnose` 路径过重或表格过多时，优先跑本脚本判断 clean estimator 是否能在统一 CRB 倍数的真值附近范围内贴近 CRB 或至少优于 SS-MF；若仍不达标，先看 `cleanKnownUnknownCompareTable` 与 `cleanCrbPathAuditAggregateTable`，再决定是否转向 scan 或回到 estimator 主流程修改，不继续往 replay 中添加候选路径。Summary output / plotting 小节只读取 `replayData` 中已保存的数据段，支持 load snapshot 后重跑。
