...
- 作用：对 canonical `4154 + 1165` 双星做固定 SNR、小 MC 的 single-frame static replay，作为 `scanSfStaticMleCrbConsistency.m` 的机制级伴随入口。
- 职责边界：scan 负责 paper-facing 的 SNR sweep 与大 repeat CRB consistency；本 replay 只看固定 SNR 下的 SS/MS static 对比、sat2 weight sweep、代表性 repeat、per-satellite static diagnostic 与 reference-sat 不变量。
...
- 主要输出：`caseSummaryTable`、`staticGainSummaryTable`、`weightPreferenceTable`、`repeatCompareTable`、`worstMsAngleLossTable`、`representativeDetail`、`staticCrbSummaryTable`、`referenceInvariantTable` 与三张 compact diagnostic 图。
- 当前结论口径：用于解释 static hard pair / weight / per-sat 机制；不替代 `scanSfStaticMleCrbConsistency` 的正文级 static anchor 曲线，也不用于改 estimator 默认路径。

#### `replaySfMsDoaCrbDiagnose.m`

- 作用：专门排查 `scanSfStaticMleCrbConsistency.m` 中 `MS-SF-DoA` 的 MLE/CRB gap，目标是区分 CRB / metric / FIM 口径问题与 DoA-only MLE solver / local-basin 问题。
- 职责边界：不替代 `scanSfStaticMleCrbConsistency.m` 的正文统计曲线，也不修改 static / dynamic estimator 默认路径；它只做小 MC 与少量 probe，用于回答“CRB 是否同口径”和“MS DoA-only MLE 是否吃到多星增益”。
- 诊断口径：同时输出 baseline `SS-SF-DoA`、`Other-SF-DoA`、baseline `MS-SF-DoA` 与 `MS-SF-DoA-TruthInit`；DoA-only truth-init 只使用 truth 作为初值，仍走完整 DoA 搜索域，不再用 `truthLocalHalfWidthDeg` 截断误差。CRB 同时报告 single / other / joint 的 single-frame DoA-Doppler CRB 与 deterministic DoA-only CRB，并额外保存 expected-vs-realized DoA gain、FIM additivity、DD/DoA-only CRB ratio、truth/final objective probe、per-sat clean/noisy numerical Hessian probe、noise-free sanity preview 和 top-tail 预览。
- 存储口径：默认不启用 checkpoint，不写 `tmp/`；外层 task loop 按 Parallel Computing Toolbox 自动使用 task-level `parfor`，保存轻量 `replayData`，summary / plot / Telegram 报告只从 `replayData` 读取，避免 load snapshot 后依赖工作区临时变量。
- 当前结论口径：若 `MS-SF-DoA-TruthInit` 仍按 DoA-only CRB 偏离，优先查 CRB/metric/FIM 或 finite-sample 口径；若 truth-init 贴 CRB 而 baseline 不贴，优先查 DoA-only solver 初值 / basin；若 MS baseline 与 SS/other 单星 RMSE 接近而 gain 表显示 realized/expected gain 明显小于 1，优先查 multi-sat DoA objective / solver path 是否真正吃到第二星信息。
...
- 固定路线：文件头 `runScale` 支持 `"ss-sanity"`、`"ms-fixed-snr"`、兼容别名 `"ms-sanity"` 与 `"full"`。当前默认 `"ms-fixed-snr"`，按 `fixedSnrDb / baseSeed / numRepeat` 做固定 SNR 连续 seed 批量 replay，当前默认 `fixedSnrDb=10`、`baseSeed=266`、`numRepeat=40`，用于先把 10 dB 的 MS tail、bank candidate 覆盖、guard adoption、soft damage 与 same-tooth 比率搞清楚，再决定是否扩大 SNR；连续 seed 在 header 中压缩为 `seedStart:seedEnd (N seeds)`，不打印完整列表。该路线保留 `SS-SF-Static / MS-SF-Static / SS-MF-CP-K/U / MS-MF-CP-K/U` 方法集，`solveProbeRoute="ms-bank-only"`，`msBankRoute="cpu-release-axis0p006-sanity"`，MS bank 只跑 static / SS-MF center 以及 `±lat/±lon @ 0.006 deg` 的最小 axis-cross CP-K preselect 与 top-1 CP-U release；不跑 truth-DoA / static-wide / SS-MF-seed deep solve，也不跑 0.012/0.024/0.048 或 diagonal bank。`ss-sanity` 仍只用于 SS-MF DoA basin-entry wiring 快速复查；`full` 才恢复代表性 seed/SNR 和完整 targeted MS 诊断。SS/MS 动态方法都会显式传入 `estimatorDoaBasinEntryScope`，由 scope 值决定哪些方法实际 evaluate entry；脚本保留 `ssDoaBasinEntrySummaryTable` 作为 compact 护栏，若 SS-MF entry 未全量 evaluated 会 warning。
- 诊断口径：`seedChainTable` 判断 MS 的坏点是 static seed 前移、MF refinement 拉坏，还是 MS 相对 SS 的 non-ref 融合问题；`objectiveProbeTable` 判断 truth / mixed point 是否优于 final；`solveProbeTable` 在 full 路线保留 baseline 行和被 targeted gate 触发的 MS deep solve 行，在 `ms-sanity` 路线只保留 baseline 与 MS bank 行；`pathProbeTable` 为兼容旧 snapshot 字段保留为空表，DoA 路径 / surface 观察交给 `replayMfCombToothSurface`、`replayMfInToothDoaDopplerRidgeTrace` 等专门 replay。
...
- 打印口径：默认 `summaryPrintLevel="compact"`，命令行只打印 raw / trim aggregate、MS primary diagnosis、wide coverage、runtime top 与代表样本等短表；`diagnostic` 才打印 seed chain、objective / solve / candidate trace、search range 等长表。完整表始终保存在 `replayData`。
