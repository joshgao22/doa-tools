...
- 固定路线：默认 `ssOnlyDebugMode=false`，恢复完整 MS 诊断：同时跑 `SS-SF-Static / MS-SF-Static / SS-MF-CP-K/U / MS-MF-CP-K/U`，MF 初始化固定为 `static-doa-truth-fd`，`objectiveProbeRoute="fixed-point-objective"`，`solveProbeRoute="ms-targeted"`，`pathProbeRoute="disabled"`，`msBankRoute="cpu-release-only"`。完整 MS 模式下 baseline / fixed-point objective / per-sat / runtime 全覆盖，controlled solve probe 只对 MS 方法按 objective / non-ref coherence / fdRef branch / first-order-opt 触发，MS bank 只服务 `MS-MF-CP-U` 的 CP-K preselect -> top-K CP-U release。SS-MF baseline 保持与 `replayMfSsMleCrbMetricDiagnose` 一致的构造方式，不额外写入 MS 专用 `doaBasinEntryScope`；脚本保留 `ssDoaBasinEntrySummaryTable` 作为 compact 护栏，若 SS-MF entry 未全量 evaluated 会 warning。若后续只想快速核对 SS parity，可临时将 `ssOnlyDebugMode=true`，此时只跑 `SS-SF-Static / SS-MF-CP-K/U`，并切到 `solveProbeRoute="ss-parity"`、`msBankRoute="disabled"`。
- 诊断口径：`seedChainTable` 判断 MS 的坏点是 static seed 前移、MF refinement 拉坏，还是 MS 相对 SS 的 non-ref 融合问题；`objectiveProbeTable` 判断 truth / mixed point 是否优于 final；`solveProbeTable` 只保留 baseline 行和被 targeted gate 触发的 MS deep solve 行；`pathProbeTable` 为兼容旧 snapshot 字段保留为空表，DoA 路径 / surface 观察交给 `replayMfCombToothSurface`、`replayMfInToothDoaDopplerRidgeTrace` 等专门 replay。
...
- 当前定位：作为 MS 估计流程剖面入口；若 scan 里 MS 不贴 CRB，应先从 `topTailExportTable` 选 seed，再用本 replay 看 seed chain、per-sat coherence/local-state、objective probe、shadow-adopted diagnostic aggregate、objective-only adoption 对照和 reject aggregate，不要把 scan 膨胀成内部 trace 脚本。`MS-MF-CP-U-BankShadowAdopt` 只回答“已有 bank candidate 若按 no-truth shadow guard 被采用，统计会怎样”，`MS-MF-CP-U-BankObjOnlyAdopt` 只回答“若仅按 objective beat 采用会怎样”；二者都不代表 estimator 默认 winner 已改变。
