...
- 作用：对照论文模型层级检查 CRB case dispatch、interest / nuisance 参数切分、time template、FIM / EFIM block、多星贡献与 summary 取值是否自洽；默认不跑 MLE，不替代性能 replay。
...
- 默认扫描：文件头默认按 `snrDbList=(-10:5:10).' × seedList` 构造 task grid；当前默认 `numRepeat=200`、seed `253:452`、`P=10`、DoA hard box `10×CRB`、MF `fdRef` truth-local `0.4 tooth` half-width。快速 smoke test 时可临时把 `numRepeat` 降到 10 或 20。该脚本已经在 aggregate、compare、search-range audit 与 CRB path audit 中按 SNR 分组；后续更系统的 DoA box scale、fdRef half-width、geometry、frame count 与 information-loss 曲线交给 scan，不再在 CleanTrim 内继续扩展扫描维度。
