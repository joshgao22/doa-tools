% replayMfInToothDoaDopplerRidgeTrace
% Purpose: trace in-tooth DoA-Doppler objective ridges around same-tooth
% dynamic estimates. The replay fixes the tooth with a truth-centered fdRef
% half-tooth box, then re-evaluates final-centered DoA-fdRef and DoA-fdRate
% surfaces along truth-free implementable directions and one oracle truth
% direction. Center-to-center line probes compare final-to-static, final-to
% single-MF, final-to-wide, and final-to-truth paths without changing solver
% adoption.
%
...
replayName = "replayMfInToothDoaDopplerRidgeTrace";
...
ridgeDoaOffsetDegList = (-0.006:0.0015:0.006).'; % Scalar DoA offsets along center directions for ridge surfaces.
ridgeFdRefOffsetHzList = (-0.08:0.02:0.08).'; % fdRef offsets used by final-centered DoA-fdRef surfaces.
ridgeFdRateOffsetHzPerSecList = (-160:40:160).'; % fdRate offsets used by final-centered DoA-fdRate surfaces.
...
tracker = localCreateProgressTracker(sprintf('%s repeat batch (%s)', char(replayName), localModeText(useParfor)), numCase, useParfor);
try
  if useParfor
...
fprintf('Running replayMfInToothDoaDopplerRidgeTrace ...\n');
...
if ~(isfinite(truthFdRefHz) && isfinite(truthFdRateHzPerSec))
  error('replayMfInToothDoaDopplerRidgeTrace:MissingTruthFdLine', ...
...
switch method.initMode
  ...
  otherwise
    error('replayMfInToothDoaDopplerRidgeTrace:UnknownInitMode', ...
...
switch method.viewMode
  ...
  otherwise
    error('replayMfInToothDoaDopplerRidgeTrace:UnknownViewMode', ...
...
switch method.doaSeedMode
  ...
  otherwise
    error('replayMfInToothDoaDopplerRidgeTrace:UnknownDoaSeedMode', ...
...
if ~(isfinite(toothStepHz) && toothStepHz > 0)
  error('replayMfInToothDoaDopplerRidgeTrace:MissingToothStep', ...
