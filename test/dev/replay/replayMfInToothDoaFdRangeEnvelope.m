...
% Purpose: scan controlled in-tooth DoA-fdRef and DoA-fdRate objective
% envelopes around implementable centers. The replay fixes the fdRef tooth with
% a truth-centered half-tooth box, then reports where each local surface reaches
% its minimum, whether the minimum hits the scan boundary, and which DoA/fdRef
% half-widths are worth trying in gated rescue.
%
...
replayName = "replayMfInToothDoaFdRangeEnvelope";

snrDb = 10;                         % Snapshot SNR used to generate rx signals.
seedMode = "base-seed-search";       % "fixed-list" uses fixedSeedList; "base-seed-search" auto-selects envelope seeds.
fixedSeedList = [253; 256; 345; 384; 420; 431; 445; 450]; % Manual fallback seed list for fixed-list mode.
baseSeed = 253;                     % First seed for the scout pass used by base-seed-search mode.
numSearchRepeat = 100;              % Number of consecutive seeds in the lightweight scout pass.
maxEnvelopeSeedCount = 12;          % Upper bound for the heavy envelope surface pass.
maxHardSeedCount = 8;               % Maximum coherence-collapsed hard seeds kept for envelope surfaces.
maxGateMissSeedCount = 3;           % Maximum hard gate-miss seeds kept for envelope surfaces.
maxEasyNegativeSeedCount = 2;       % Maximum easy negative controls kept for damage checks.
...
probeFdRefStepHzList = [-300; -150; 0; 150; 300];      % In-tooth fdRef offsets for local joint probes.
probeLineAlphaList = [0; 0.25; 0.5; 0.75; 0.85; 0.90; 0.95; 0.98; 0.99; 1]; % Line probe interpolation factors toward truth DoA.
probeCoarseDoaStepDegList = [-0.008; -0.006; -0.004; -0.002; 0; 0.002; 0.004; 0.006; 0.008]; % Implementable non-truth center sweep offsets.
ridgeDoaOffsetDegList = (-0.012:0.002:0.012).'; % Scalar DoA offsets along center directions for range envelope surfaces.
ridgeFdRefOffsetHzList = (-360:60:360).'; % In-tooth fdRef offsets used by DoA-fdRef envelope surfaces.
ridgeFdRateOffsetHzPerSecList = (-240:80:240).'; % fdRate offsets used by DoA-fdRate envelope surfaces.
ridgeLineAlphaList = [0; 0.10; 0.25; 0.50; 0.75; 0.90; 1.00]; % Center-to-center line probe interpolation factors.
...
angleHitThresholdDeg = 0.002;       % Offline DoA hit threshold used by coverage and policy summaries.
...
fixedSeedList = reshape(double(fixedSeedList), [], 1);
searchSeedList = reshape(double(baseSeed + (0:(numSearchRepeat - 1))), [], 1);
if seedMode == "fixed-list"
  seedList = fixedSeedList;
else
...
config = struct();
config.snrDb = snrDb;
config.seedMode = string(seedMode);
config.fixedSeedList = fixedSeedList;
config.baseSeed = baseSeed;
config.numSearchRepeat = numSearchRepeat;
config.searchSeedList = searchSeedList;
config.maxEnvelopeSeedCount = maxEnvelopeSeedCount;
config.maxHardSeedCount = maxHardSeedCount;
config.maxGateMissSeedCount = maxGateMissSeedCount;
config.maxEasyNegativeSeedCount = maxEasyNegativeSeedCount;
config.maxFdNegativeSeedCount = maxFdNegativeSeedCount;
...
config.checkpointCleanupOnSuccess = checkpointCleanupOnSuccess;
...
config.ridgeDoaOffsetDegList = reshape(double(ridgeDoaOffsetDegList), [], 1);
config.ridgeFdRefOffsetHzList = reshape(double(ridgeFdRefOffsetHzList), [], 1);
config.ridgeFdRateOffsetHzPerSecList = reshape(double(ridgeFdRateOffsetHzPerSecList), [], 1);
config.ridgeLineAlphaList = reshape(double(ridgeLineAlphaList), [], 1);
...
try
  if config.seedMode == "base-seed-search"
    scoutConfig = config;
    scoutConfig.seedList = config.searchSeedList;
    scoutConfig.numCase = numel(config.searchSeedList);
    scoutConfig.skipCandidateProbe = true;
    [scoutRepeatCell, scoutRunState] = localRunRepeatBatch(scoutConfig.seedList, "scout", context, flowOpt, methodList, scoutConfig, replayName);
    checkpointStateList = localAppendCheckpointState(checkpointStateList, scoutRunState);
    scoutMethodTable = localBuildMethodTable(scoutRepeatCell);
    scoutTailDiagnosisTable = localBuildTailDiagnosisTable(scoutMethodTable, scoutConfig);
    selectionOpt = struct( ...
      'maxEnvelopeSeedCount', config.maxEnvelopeSeedCount, ...
      'maxHardSeedCount', config.maxHardSeedCount, ...
      'maxGateMissSeedCount', config.maxGateMissSeedCount, ...
      'maxEasyNegativeSeedCount', config.maxEasyNegativeSeedCount, ...
      'maxFdNegativeSeedCount', config.maxFdNegativeSeedCount, ...
      'gatedRescueCoherenceThreshold', config.gatedRescueCoherenceThreshold, ...
      'truthDoaGapThresholdDeg', config.truthDoaGapThresholdDeg);
    selectedEnvelopeSeedTable = selectMfInToothEnvelopeSeeds(scoutTailDiagnosisTable, selectionOpt);
    if isempty(selectedEnvelopeSeedTable)
      error('replayMfInToothDoaFdRangeEnvelope:NoSelectedEnvelopeSeed', ...
        'The base-seed scout did not select any seed for the heavy envelope pass.');
    end
    config.seedList = reshape(double(selectedEnvelopeSeedTable.taskSeed), [], 1);
    config.numCase = numel(config.seedList);
    fprintf('[%s] Selected %d envelope seeds from the scout pass: %s\n', ...
      char(datetime('now', 'Format', 'HH:mm:ss')), config.numCase, mat2str(config.seedList(:).'));
  else
    selectedEnvelopeSeedTable = localBuildFixedSelectedSeedTable(config.seedList);
  end

  probeConfig = config;
  probeConfig.skipCandidateProbe = false;
  [repeatCell, envelopeRunState] = localRunRepeatBatch(config.seedList, "envelope", context, flowOpt, methodList, probeConfig, replayName);
  checkpointStateList = localAppendCheckpointState(checkpointStateList, envelopeRunState);
catch ME
...
ridgeSummaryTable = localBuildRidgeSummaryTable(candidateProbeTable, config);
centerLineSummaryTable = localBuildCenterLineSummaryTable(candidateProbeTable);
envelopeSummaryTable = localBuildEnvelopeSummaryTable(candidateProbeTable, tailDiagnosisTable, config);
rangeRecommendationTable = localBuildRangeRecommendationTable(envelopeSummaryTable);
candidateWinnerTable = localBuildCandidateWinnerTable(candidateProbeTable, tailDiagnosisTable, config);
candidateCoverageTable = localBuildCandidateCoverageTable(candidateWinnerTable, config);
policyRecommendationTable = localBuildPolicyRecommendationTable(candidateProbeTable, tailDiagnosisTable, config);
...
plotData.candidateProbeTable = candidateProbeTable;
plotData.ridgeSummaryTable = ridgeSummaryTable;
plotData.centerLineSummaryTable = centerLineSummaryTable;
plotData.envelopeSummaryTable = envelopeSummaryTable;
plotData.rangeRecommendationTable = rangeRecommendationTable;
plotData.candidateWinnerTable = candidateWinnerTable;
plotData.candidateCoverageTable = candidateCoverageTable;
plotData.policyRecommendationTable = policyRecommendationTable;
...
replayData.selectedEnvelopeSeedTable = selectedEnvelopeSeedTable;
replayData.scoutMethodTable = scoutMethodTable;
replayData.scoutTailDiagnosisTable = scoutTailDiagnosisTable;
...
replayData.ridgeSummaryTable = ridgeSummaryTable;
replayData.centerLineSummaryTable = centerLineSummaryTable;
replayData.envelopeSummaryTable = envelopeSummaryTable;
replayData.rangeRecommendationTable = rangeRecommendationTable;
replayData.candidateWinnerTable = candidateWinnerTable;
replayData.candidateCoverageTable = candidateCoverageTable;
replayData.policyRecommendationTable = policyRecommendationTable;
...
if config.checkpointEnable && config.checkpointCleanupOnSuccess
  replayData.checkpointSummary.cleanupReport = localCleanupCheckpointStateList(checkpointStateList);
...
if isfield(replayData, 'ridgeSummaryTable')
  ridgeSummaryTable = replayData.ridgeSummaryTable;
else
  ridgeSummaryTable = localBuildRidgeSummaryTable(candidateProbeTable, replayData.config);
end
if isfield(replayData, 'centerLineSummaryTable')
  centerLineSummaryTable = replayData.centerLineSummaryTable;
else
  centerLineSummaryTable = localBuildCenterLineSummaryTable(candidateProbeTable);
end
if isfield(replayData, 'envelopeSummaryTable')
  envelopeSummaryTable = replayData.envelopeSummaryTable;
else
  envelopeSummaryTable = localBuildEnvelopeSummaryTable(candidateProbeTable, tailDiagnosisTable, replayData.config);
end
if isfield(replayData, 'rangeRecommendationTable')
  rangeRecommendationTable = replayData.rangeRecommendationTable;
else
  rangeRecommendationTable = localBuildRangeRecommendationTable(envelopeSummaryTable);
end
...
if isfield(replayData, 'candidateCoverageTable')
  candidateCoverageTable = replayData.candidateCoverageTable;
else
  candidateCoverageTable = localBuildCandidateCoverageTable(candidateWinnerTable, replayData.config);
end
if isfield(replayData, 'policyRecommendationTable')
  policyRecommendationTable = replayData.policyRecommendationTable;
else
  policyRecommendationTable = localBuildPolicyRecommendationTable(candidateProbeTable, tailDiagnosisTable, replayData.config);
end
...
if ~isempty(ridgeSummaryTable)
  fprintf('\n========== In-tooth ridge surface summary ==========\n');
  disp(ridgeSummaryTable);
end
if ~isempty(centerLineSummaryTable)
  fprintf('\n========== Center-to-center line trace summary ==========\n');
  disp(centerLineSummaryTable);
end
if ~isempty(candidateWinnerTable)
  fprintf('\n========== Tail candidate winner summary ==========\n');
  disp(candidateWinnerTable);
end
if ~isempty(candidateCoverageTable)
  fprintf('\n========== Tail candidate coverage summary ==========\n');
  disp(candidateCoverageTable);
end
if ~isempty(policyRecommendationTable)
  fprintf('\n========== Tail policy recommendation summary ==========\n');
  disp(policyRecommendationTable);
end
...
function [repeatCell, runState] = localRunRepeatBatch(seedList, stageLabel, context, flowOpt, methodList, config, replayName)
%LOCALRUNREPEATBATCH Run a scout or heavy envelope repeat batch with optional checkpointing.

seedList = reshape(double(seedList), [], 1);
numCase = numel(seedList);
repeatCell = cell(numCase, 1);
runState = localEmptyCheckpointState();
useParfor = localCanUseParfor() && numCase > 1;
if localCheckpointEnabled(config)
  checkpointOpt = localBuildCheckpointOpt(replayName, stageLabel, seedList, config, useParfor);
  ...
  numDoneTask = localCountCheckpointTaskFile(fullfile(checkpointOpt.runDir, 'task'), numCase);
  numTodoTask = numCase - numDoneTask;
  tracker = localCreateProgressTracker(sprintf('%s %s batch (%s, resume %d/%d)', ...
    char(replayName), char(stageLabel), localModeText(useParfor), numDoneTask, numCase), numTodoTask, false);
  ...
  if ~runState.isComplete
    error('replayMfInToothDoaFdRangeEnvelope:CheckpointIncomplete', ...
      'Checkpoint replay returned an incomplete %s batch.', char(stageLabel));
  end
  repeatCell = runState.resultCell;
  return;
end

tracker = localCreateProgressTracker(sprintf('%s %s batch (%s)', ...
  char(replayName), char(stageLabel), localModeText(useParfor)), numCase, useParfor);
try
  if useParfor
    progressQueue = tracker.queue;
    parfor iRepeat = 1:numCase
...
function runKey = localBuildCheckpointRunKey(stageLabel, seedList, config)
%LOCALBUILDCHECKPOINTRUNKEY Build a stable key that changes with envelope settings.

seedList = reshape(double(seedList), 1, []);
if isempty(seedList)
  seedPart = 'empty';
else
  seedPart = sprintf('seed%dto%d_rep%d', seedList(1), seedList(end), numel(seedList));
end
probePart = sprintf('probe%d', ~logical(localGetFieldOrDefault(config, 'skipCandidateProbe', false)));
runKey = string(sprintf('%s_%s_snr%.2f_ctx%d_fdfrac%.3f_fdrate%.0f_%s_doa%d_fd%d_fdrate%d', ...
  char(stageLabel), seedPart, config.snrDb, config.contextBaseSeed, ...
  config.oracleFdHalfToothFraction, config.oracleFdRateHalfWidthHzPerSec, ...
  probePart, numel(config.ridgeDoaOffsetDegList), numel(config.ridgeFdRefOffsetHzList), ...
  numel(config.ridgeFdRateOffsetHzPerSecList)));
runKey = replace(runKey, '.', 'p');
runKey = replace(runKey, '-', 'm');
end

function meta = localBuildCheckpointMeta(stageLabel, seedList, config)
%LOCALBUILDCHECKPOINTMETA Store compact fields that affect checkpoint compatibility.

meta = struct();
meta.stageLabel = string(stageLabel);
meta.snrDb = config.snrDb;
meta.seedMode = string(config.seedMode);
meta.seedList = reshape(double(seedList), 1, []);
meta.contextBaseSeed = config.contextBaseSeed;
meta.skipCandidateProbe = logical(localGetFieldOrDefault(config, 'skipCandidateProbe', false));
meta.oracleFdHalfToothFraction = config.oracleFdHalfToothFraction;
meta.oracleFdRateHalfWidthHzPerSec = config.oracleFdRateHalfWidthHzPerSec;
meta.staticLocalDoaHalfWidthDeg = reshape(double(config.staticLocalDoaHalfWidthDeg), 1, []);
meta.staticWideDoaHalfWidthDeg = reshape(double(config.staticWideDoaHalfWidthDeg), 1, []);
meta.truthLocalDoaHalfWidthDeg = reshape(double(config.truthLocalDoaHalfWidthDeg), 1, []);
meta.fdRefHealthyAbsHz = config.fdRefHealthyAbsHz;
meta.fdRateHealthyAbsHzPerSec = config.fdRateHealthyAbsHzPerSec;
meta.coherenceCollapseThreshold = config.coherenceCollapseThreshold;
meta.truthDoaGapThresholdDeg = config.truthDoaGapThresholdDeg;
meta.probeDoaStepDegList = reshape(double(config.probeDoaStepDegList), 1, []);
meta.probeFdRefStepHzList = reshape(double(config.probeFdRefStepHzList), 1, []);
meta.probeCoarseDoaStepDegList = reshape(double(config.probeCoarseDoaStepDegList), 1, []);
meta.ridgeDoaOffsetDegList = reshape(double(config.ridgeDoaOffsetDegList), 1, []);
meta.ridgeFdRefOffsetHzList = reshape(double(config.ridgeFdRefOffsetHzList), 1, []);
meta.ridgeFdRateOffsetHzPerSecList = reshape(double(config.ridgeFdRateOffsetHzPerSecList), 1, []);
meta.ridgeLineAlphaList = reshape(double(config.ridgeLineAlphaList), 1, []);
meta.gatedRescueCoherenceThreshold = config.gatedRescueCoherenceThreshold;
meta.gatedRescuePhaseResidThresholdRad = config.gatedRescuePhaseResidThresholdRad;
end
...
%LOCALCHECKPOINTTASKRUNNER Run one checkpointed envelope repeat task.
...
%LOCALBUILDCHECKPOINTRUNNEROPT Keep only fields accepted by runPerfTaskGridWithCheckpoint.
...
function state = localEmptyCheckpointState()
%LOCALEMPTYCHECKPOINTSTATE Return an empty checkpoint run-state placeholder.

state = struct('runName', "", 'runKey', "", 'runDir', "", 'numTask', 0, ...
  'numDone', 0, 'isComplete', false, 'cleanedOnSuccess', false);
end

function stateList = localAppendCheckpointState(stateList, runState)
%LOCALAPPENDCHECKPOINTSTATE Append nonempty checkpoint state to a state list.

if ~isstruct(runState) || ~isfield(runState, 'runDir') || strlength(string(runState.runDir)) == 0
  return;
end
...
function cleanupReport = localCleanupCheckpointStateList(stateList)
%LOCALCLEANUPCHECKPOINTSTATELIST Delete all completed checkpoint run directories.
...
for iState = 1:numel(stateList)
  ...
  reportUse = cleanupPerfTaskGridCheckpoint(stateList(iState), struct('verbose', false));
  if isempty(reportUse)
    continue;
  end
  if isempty(cleanupReport)
    cleanupReport = reportUse(:);
  else
    cleanupReport = [cleanupReport(:); reportUse(:)]; %#ok<AGROW>
  end
end
end

function localPrintCheckpointFailure(stateList, replayName, config)
%LOCALPRINTCHECKPOINTFAILURE Print checkpoint directories kept after failure.

fprintf('Replay failed. Checkpoint artifacts were kept for resume.\n');
if ~isempty(stateList)
  for iState = 1:numel(stateList)
    runDir = string(localGetFieldOrDefault(stateList(iState), 'runDir', ""));
...
if logical(localGetFieldOrDefault(config, 'checkpointEnable', false))
  checkpointRoot = fullfile(localGetRepoRoot(), 'tmp', char(replayName));
  fprintf('  checkpoint parent: %s\n', checkpointRoot);
end
end

function localPrintCheckpointSummary(checkpointSummary)
%LOCALPRINTCHECKPOINTSUMMARY Print compact checkpoint paths after reload or completion.

if ~isstruct(checkpointSummary) || ~isfield(checkpointSummary, 'runDir')
  return;
end
for iState = 1:numel(checkpointSummary.runDir)
  if strlength(string(checkpointSummary.runDir(iState))) > 0
    fprintf('  checkpoint dir (%d)              : %s\n', iState, char(checkpointSummary.runDir(iState)));
  end
end
end

function selectedSeedTable = localBuildFixedSelectedSeedTable(seedList)
%LOCALBUILDFIXEDSELECTEDSEEDTABLE Wrap manual seed list in the same table shape as base-seed selection.
...
selectedSeedTable = table(seedList, repmat("fixed-list", numel(seedList), 1), ...
  nan(numel(seedList), 1), repmat("manual", numel(seedList), 1), ...
  'VariableNames', {'taskSeed', 'selectionRole', 'selectionMetric', 'tailClass'});
end
...
if ~(isfinite(truthFdRefHz) && isfinite(truthFdRateHzPerSec))
  error('replayMfInToothDoaFdRangeEnvelope:MissingTruthFdLine', ...
...
if localGetFieldOrDefault(config, 'skipCandidateProbe', false)
  candidateProbeTable = table();
else
...
switch method.initMode
  ...
  otherwise
    error('replayMfInToothDoaFdRangeEnvelope:UnknownInitMode', ...
...
switch method.viewMode
  ...
  otherwise
    error('replayMfInToothDoaFdRangeEnvelope:UnknownViewMode', ...
...
switch method.doaSeedMode
  ...
  otherwise
    error('replayMfInToothDoaFdRangeEnvelope:UnknownDoaSeedMode', ...
...
ridgeDoaOffsetDegList = reshape(localGetFieldOrDefault(config, 'ridgeDoaOffsetDegList', (-0.006:0.0015:0.006).'), [], 1);
ridgeFdRefOffsetHzList = reshape(localGetFieldOrDefault(config, 'ridgeFdRefOffsetHzList', (-0.08:0.02:0.08).'), [], 1);
ridgeFdRateOffsetHzPerSecList = reshape(localGetFieldOrDefault(config, 'ridgeFdRateOffsetHzPerSecList', (-160:40:160).'), [], 1);
ridgeLineAlphaList = reshape(localGetFieldOrDefault(config, 'ridgeLineAlphaList', [0; 0.10; 0.25; 0.50; 0.75; 0.90; 1.00]), [], 1);
truthCenterOptVar = defaultOptVar;
truthCenterOptVar(1:2) = truthDoaParam(1:2);
truthCenterOptVar(3:4) = truthLineFd(1:2);
centerLabelList = ["static-ms"; "single-mf"; "wide"; "truth"];
centerOptCell = {staticCenterOptVar; singleOptVar; wideOptVar; truthCenterOptVar};
for iCenter = 1:numel(centerLabelList)
  centerOptVar = centerOptCell{iCenter};
  if numel(centerOptVar) ~= numel(defaultOptVar)
    continue;
  end
  rowCell = localAppendDirectionalRidgeSurface(rowCell, iRepeat, taskSeed, centerLabelList(iCenter), ...
    centerOptVar, defaultOptVar, ridgeDoaOffsetDegList, ridgeFdRefOffsetHzList, ...
    ridgeFdRateOffsetHzPerSecList, model, truth, toothStepHz);
  rowCell = localAppendCenterLineTrace(rowCell, iRepeat, taskSeed, centerLabelList(iCenter), ...
    centerOptVar, defaultOptVar, ridgeLineAlphaList, model, truth, toothStepHz);
end
...
function rowCell = localAppendDirectionalRidgeSurface(rowCell, iRepeat, taskSeed, directionLabel, centerOptVar, defaultOptVar, doaOffsetDegList, fdRefOffsetHzList, fdRateOffsetHzPerSecList, model, truth, toothStepHz)
%LOCALAPPENDDIRECTIONALRIDGESURFACE Add DoA-fdRef and DoA-fdRate surface probes.

centerOptVar = reshape(double(centerOptVar), [], 1);
defaultOptVar = reshape(double(defaultOptVar), [], 1);
doaOffsetDegList = reshape(double(doaOffsetDegList), [], 1);
fdRefOffsetHzList = reshape(double(fdRefOffsetHzList), [], 1);
fdRateOffsetHzPerSecList = reshape(double(fdRateOffsetHzPerSecList), [], 1);
if numel(centerOptVar) < 4 || numel(defaultOptVar) < 4
  return;
end
if any(~isfinite(centerOptVar(1:4))) || any(~isfinite(defaultOptVar(1:4)))
  return;
end
[directionUnit, directionNormDeg] = localBuildDoaDirectionUnit(centerOptVar(1:2), defaultOptVar(1:2));
for iDoa = 1:numel(doaOffsetDegList)
  doaOffsetDeg = doaOffsetDegList(iDoa);
  if ~isfinite(doaOffsetDeg)
    continue;
  end
  for iFd = 1:numel(fdRefOffsetHzList)
    fdRefOffsetHz = fdRefOffsetHzList(iFd);
    if ~isfinite(fdRefOffsetHz)
      continue;
    end
    optVar = defaultOptVar;
    optVar(1:2) = defaultOptVar(1:2) + directionUnit(:) * doaOffsetDeg;
    optVar(3) = defaultOptVar(3) + fdRefOffsetHz;
    tag = sprintf('dir-%s-doa%+.4g-fdref%+.4g', char(directionLabel), doaOffsetDeg, fdRefOffsetHz);
    row = localEvaluateCandidateProbePoint(iRepeat, taskSeed, "ridge-doa-fdref-surface", ...
      string(tag), "ridge-surface", optVar, optVar(1:2) - defaultOptVar(1:2), ...
      fdRefOffsetHz, optVar(4) - defaultOptVar(4), model, truth, toothStepHz);
    row.surfaceKind = "doa-fdref";
    row.ridgeDirectionFamily = string(directionLabel);
    row.ridgeDoaOffsetDeg = doaOffsetDeg;
    row.directionNormDeg = directionNormDeg;
    row.directionUnit1 = directionUnit(1);
    row.directionUnit2 = directionUnit(2);
    rowCell{end + 1, 1} = row; %#ok<AGROW>
  end
  for iRate = 1:numel(fdRateOffsetHzPerSecList)
    fdRateOffsetHzPerSec = fdRateOffsetHzPerSecList(iRate);
    if ~isfinite(fdRateOffsetHzPerSec)
      continue;
    end
    optVar = defaultOptVar;
    optVar(1:2) = defaultOptVar(1:2) + directionUnit(:) * doaOffsetDeg;
    optVar(4) = defaultOptVar(4) + fdRateOffsetHzPerSec;
    tag = sprintf('dir-%s-doa%+.4g-fdrate%+.4g', char(directionLabel), doaOffsetDeg, fdRateOffsetHzPerSec);
    row = localEvaluateCandidateProbePoint(iRepeat, taskSeed, "ridge-doa-fdrate-surface", ...
      string(tag), "ridge-surface", optVar, optVar(1:2) - defaultOptVar(1:2), ...
      optVar(3) - defaultOptVar(3), fdRateOffsetHzPerSec, model, truth, toothStepHz);
    row.surfaceKind = "doa-fdrate";
    row.ridgeDirectionFamily = string(directionLabel);
    row.ridgeDoaOffsetDeg = doaOffsetDeg;
    row.directionNormDeg = directionNormDeg;
    row.directionUnit1 = directionUnit(1);
...
function rowCell = localAppendCenterLineTrace(rowCell, iRepeat, taskSeed, directionLabel, centerOptVar, defaultOptVar, alphaList, model, truth, toothStepHz)
%LOCALAPPENDCENTERLINETRACE Add final-to-center line probes for one direction.

centerOptVar = reshape(double(centerOptVar), [], 1);
defaultOptVar = reshape(double(defaultOptVar), [], 1);
alphaList = reshape(double(alphaList), [], 1);
if numel(centerOptVar) < 4 || numel(defaultOptVar) < 4
  return;
end
if any(~isfinite(centerOptVar(1:4))) || any(~isfinite(defaultOptVar(1:4)))
...
for iAlpha = 1:numel(alphaList)
  ...
  optVar = (1 - alpha) * defaultOptVar + alpha * centerOptVar;
  tag = sprintf('final-to-%s-alpha%.4g', char(directionLabel), alpha);
  row = localEvaluateCandidateProbePoint(iRepeat, taskSeed, "center-line-trace", ...
    string(tag), "ridge-line", optVar, optVar(1:2) - defaultOptVar(1:2), ...
    optVar(3) - defaultOptVar(3), optVar(4) - defaultOptVar(4), model, truth, toothStepHz);
  row.surfaceKind = "center-line";
  row.ridgeDirectionFamily = string(directionLabel);
  row.ridgeDoaOffsetDeg = alpha * directionNormDeg;
  row.directionNormDeg = directionNormDeg;
  row.directionUnit1 = directionUnit(1);
  row.directionUnit2 = directionUnit(2);
  row.probeCenterFamily = string(directionLabel) + "-center-line";
  row.lineAlpha = alpha;
  row.lineStartFamily = "final";
  row.fdLineSource = string(directionLabel) + "-center";
  rowCell{end + 1, 1} = row; %#ok<AGROW>
end
end

function [directionUnit, directionNormDeg] = localBuildDoaDirectionUnit(centerDoaDeg, defaultDoaDeg)
%LOCALBUILDDOADIRECTIONUNIT Build a scalar DoA-axis unit from final to a center.

direction = reshape(double(centerDoaDeg(1:2)), [], 1) - reshape(double(defaultDoaDeg(1:2)), [], 1);
directionNormDeg = sqrt(sum(direction .^ 2));
if ~(isfinite(directionNormDeg) && directionNormDeg > eps)
  directionUnit = [1; 0];
  directionNormDeg = 0;
else
  directionUnit = direction(:) / directionNormDeg;
end
end
...
row.fdLineSource = "";
row.surfaceKind = "";
row.ridgeDirectionFamily = "";
row.ridgeDoaOffsetDeg = NaN;
row.directionNormDeg = NaN;
row.directionUnit1 = NaN;
row.directionUnit2 = NaN;
...
function envelopeSummaryTable = localBuildEnvelopeSummaryTable(candidateProbeTable, tailDiagnosisTable, config)
%LOCALBUILDENVELOPESUMMARYTABLE Summarize local surface minima and boundary hits.

envelopeSummaryTable = table();
if isempty(candidateProbeTable) || ~ismember('surfaceKind', candidateProbeTable.Properties.VariableNames)
  return;
end
mask = candidateProbeTable.candidateFamily == "ridge-doa-fdref-surface" | ...
  candidateProbeTable.candidateFamily == "ridge-doa-fdrate-surface";
probeTable = candidateProbeTable(mask, :);
if isempty(probeTable)
  return;
end
keyTable = unique(probeTable(:, {'taskSeed', 'ridgeDirectionFamily', 'surfaceKind'}), 'rows', 'stable');
rowCell = cell(height(keyTable), 1);
for iKey = 1:height(keyTable)
  key = keyTable(iKey, :);
  keyMask = probeTable.taskSeed == key.taskSeed & ...
    probeTable.ridgeDirectionFamily == key.ridgeDirectionFamily & ...
    probeTable.surfaceKind == key.surfaceKind & isfinite(probeTable.obj);
  subTable = probeTable(keyMask, :);
  [~, idxBest] = min(subTable.obj);
  best = subTable(idxBest, :);
  defaultObj = localDefaultObjForSeed(candidateProbeTable, key.taskSeed(1));
  caseRole = localLookupCaseRole(tailDiagnosisTable, key.taskSeed(1));
  surfaceKind = string(key.surfaceKind);
  offsetVec = localGetEnvelopeOffsetVector(config, surfaceKind);
  fdOffset = NaN;
  fdRateOffset = NaN;
  if surfaceKind == "doa-fdref"
    fdOffset = best.fdRefStepHz(1);
    boundaryHit = localIsBoundary(best.ridgeDoaOffsetDeg(1), config.ridgeDoaOffsetDegList) || ...
      localIsBoundary(fdOffset, offsetVec);
  else
    fdRateOffset = best.fdRateStepHzPerSec(1);
    boundaryHit = localIsBoundary(best.ridgeDoaOffsetDeg(1), config.ridgeDoaOffsetDegList) || ...
      localIsBoundary(fdRateOffset, offsetVec);
  end
  row = struct();
  row.taskSeed = key.taskSeed(1);
  row.caseRole = caseRole;
  row.ridgeDirectionFamily = string(key.ridgeDirectionFamily);
  row.surfaceKind = surfaceKind;
  row.minDoaOffsetDeg = best.ridgeDoaOffsetDeg(1);
  row.minFdRefOffsetHz = fdOffset;
  row.minFdRateOffsetHzPerSec = fdRateOffset;
  row.minAngleErrDeg = best.angleErrDeg(1);
  row.minNonRefCoherenceFloor = best.nonRefCoherenceFloor(1);
  row.minObjGainFromDefault = defaultObj - best.obj(1);
  row.boundaryHit = logical(boundaryHit);
  row.directionNormDeg = best.directionNormDeg(1);
  row.directionUnit1 = best.directionUnit1(1);
  row.directionUnit2 = best.directionUnit2(1);
  rowCell{iKey} = row;
end
envelopeSummaryTable = struct2table([rowCell{:}].');
end

function rangeRecommendationTable = localBuildRangeRecommendationTable(envelopeSummaryTable)
%LOCALBUILDRANGERECOMMENDATIONTABLE Convert envelope minima into compact range hints.

rangeRecommendationTable = table();
if isempty(envelopeSummaryTable)
  return;
end
keyTable = unique(envelopeSummaryTable(:, {'ridgeDirectionFamily', 'surfaceKind'}), 'rows', 'stable');
rowCell = cell(height(keyTable), 1);
for iKey = 1:height(keyTable)
  key = keyTable(iKey, :);
  mask = envelopeSummaryTable.ridgeDirectionFamily == key.ridgeDirectionFamily & ...
    envelopeSummaryTable.surfaceKind == key.surfaceKind;
  subTable = envelopeSummaryTable(mask, :);
  hardMask = subTable.caseRole == "hard-collapse";
  if any(hardMask)
    useTable = subTable(hardMask, :);
  else
    useTable = subTable;
  end
  row = struct();
  row.ridgeDirectionFamily = string(key.ridgeDirectionFamily);
  row.surfaceKind = string(key.surfaceKind);
  row.numCase = height(subTable);
  row.numHardCase = nnz(hardMask);
  row.boundaryHitRate = mean(double(subTable.boundaryHit), 'omitnan');
  row.hardBoundaryHitRate = mean(double(subTable.boundaryHit(hardMask)), 'omitnan');
  row.p95AbsDoaOffsetDeg = localPercentile(abs(useTable.minDoaOffsetDeg), 95);
  if row.surfaceKind == "doa-fdref"
    row.p95AbsFdOffsetHz = localPercentile(abs(useTable.minFdRefOffsetHz), 95);
    row.p95AbsFdRateOffsetHzPerSec = NaN;
  else
    row.p95AbsFdOffsetHz = NaN;
    row.p95AbsFdRateOffsetHzPerSec = localPercentile(abs(useTable.minFdRateOffsetHzPerSec), 95);
  end
  row.medianObjGainFromDefault = median(useTable.minObjGainFromDefault, 'omitnan');
  row.suggestedDoaHalfWidthDeg = localRoundUpRange(row.p95AbsDoaOffsetDeg, [0.004, 0.006, 0.008, 0.010, 0.012]);
  row.suggestedFdHalfWidthHz = localRoundUpRange(row.p95AbsFdOffsetHz, [150, 300, 450]);
  row.suggestedFdRateHalfWidthHzPerSec = localRoundUpRange(row.p95AbsFdRateOffsetHzPerSec, [160, 240, 320]);
  if row.boundaryHitRate > 0.25
    row.recommendation = "expand-before-adopt";
  elseif row.surfaceKind == "doa-fdref" && isfinite(row.p95AbsFdOffsetHz) && row.p95AbsFdOffsetHz > 0
    row.recommendation = "candidate-joint-doa-fdref-range";
  else
    row.recommendation = "diagnostic-only";
  end
  rowCell{iKey} = row;
end
rangeRecommendationTable = struct2table([rowCell{:}].');
end

function value = localDefaultObjForSeed(candidateProbeTable, taskSeed)
%LOCALDEFAULTOBJFORSEED Read the default objective used as the local envelope baseline.

value = NaN;
mask = candidateProbeTable.taskSeed == taskSeed & candidateProbeTable.candidateFamily == "default-final";
if any(mask)
  idx = find(mask, 1, 'first');
  value = candidateProbeTable.obj(idx);
end
end

function caseRole = localLookupCaseRole(tailDiagnosisTable, taskSeed)
%LOCALLOOKUPCASEROLE Return the offline case role for one seed.

caseRole = "unknown";
if isempty(tailDiagnosisTable) || ~ismember('taskSeed', tailDiagnosisTable.Properties.VariableNames)
  return;
end
mask = tailDiagnosisTable.taskSeed == taskSeed;
if any(mask) && ismember('caseRole', tailDiagnosisTable.Properties.VariableNames)
  caseRole = tailDiagnosisTable.caseRole(find(mask, 1, 'first'));
end
end

function offsetVec = localGetEnvelopeOffsetVector(config, surfaceKind)
%LOCALGETENVELOPEOFFSETVECTOR Resolve the offset grid for one surface kind.

if string(surfaceKind) == "doa-fdref"
  offsetVec = reshape(config.ridgeFdRefOffsetHzList, [], 1);
else
  offsetVec = reshape(config.ridgeFdRateOffsetHzPerSecList, [], 1);
end
end

function isBoundary = localIsBoundary(value, gridVec)
%LOCALISBOUNDARY Return true when a surface minimum hits a grid edge.

gridVec = reshape(double(gridVec), [], 1);
if isempty(gridVec) || ~isfinite(value)
  isBoundary = false;
  return;
end
isBoundary = abs(value - min(gridVec)) <= 10 * eps(max(1, abs(value))) || ...
  abs(value - max(gridVec)) <= 10 * eps(max(1, abs(value)));
end

function roundedValue = localRoundUpRange(value, candidateList)
%LOCALROUNDUPRANGE Round a percentile offset up to the nearest candidate range.

roundedValue = NaN;
if ~(isfinite(value) && value >= 0)
  return;
end
candidateList = reshape(double(candidateList), [], 1);
idx = find(candidateList >= value, 1, 'first');
if isempty(idx)
  roundedValue = candidateList(end);
else
  roundedValue = candidateList(idx);
end
end

function ridgeSummaryTable = localBuildRidgeSummaryTable(candidateProbeTable, config)
%LOCALBUILDRIDGESUMMARYTABLE Summarize best points on each ridge surface.

if isempty(candidateProbeTable) || ~ismember('surfaceKind', candidateProbeTable.Properties.VariableNames)
  ridgeSummaryTable = table();
  return;
end
surfaceMask = ismember(string(candidateProbeTable.surfaceKind), ["doa-fdref"; "doa-fdrate"]);
if ~any(surfaceMask)
  ridgeSummaryTable = table();
  return;
end
surfaceTable = candidateProbeTable(surfaceMask, :);
keyTable = unique(surfaceTable(:, {'taskSeed', 'surfaceKind', 'ridgeDirectionFamily'}), 'rows', 'stable');
rowCell = cell(height(keyTable), 1);
for iKey = 1:height(keyTable)
  seedNow = keyTable.taskSeed(iKey);
  surfaceKindNow = string(keyTable.surfaceKind(iKey));
  directionNow = string(keyTable.ridgeDirectionFamily(iKey));
  mask = surfaceTable.taskSeed == seedNow & string(surfaceTable.surfaceKind) == surfaceKindNow & ...
    string(surfaceTable.ridgeDirectionFamily) == directionNow & isfinite(surfaceTable.obj);
  idx = find(mask);
  [~, idxRel] = min(surfaceTable.obj(idx));
  bestRow = surfaceTable(idx(idxRel), :);
  defaultRow = localBestProbeRow(candidateProbeTable(candidateProbeTable.taskSeed == seedNow, :), "default-final");
  row = struct();
  row.taskSeed = seedNow;
  row.surfaceKind = surfaceKindNow;
  row.ridgeDirectionFamily = directionNow;
  row.truthFreeDirection = directionNow ~= "truth";
  row.bestObjGainFromDefault = localProbeValue(bestRow, 'objGainFromDefault', NaN);
  ...
  row.bestAngleGainFromDefaultDeg = row.defaultAngleErrDeg - row.bestAngleErrDeg;
  row.bestNonRefCoherenceFloor = localProbeValue(bestRow, 'nonRefCoherenceFloor', NaN);
  row.bestDoaOffsetDeg = localProbeValue(bestRow, 'ridgeDoaOffsetDeg', NaN);
  row.bestDoaStep1Deg = localProbeValue(bestRow, 'doaStep1Deg', NaN);
  row.bestDoaStep2Deg = localProbeValue(bestRow, 'doaStep2Deg', NaN);
  row.bestFdRefOffsetHz = localProbeValue(bestRow, 'fdRefStepHz', NaN);
  row.bestFdRateOffsetHzPerSec = localProbeValue(bestRow, 'fdRateStepHzPerSec', NaN);
  row.bestToothResidualHz = localProbeValue(bestRow, 'toothResidualHz', NaN);
  row.directionNormDeg = localProbeValue(bestRow, 'directionNormDeg', NaN);
  row.ridgeSlopeHzPerDeg = NaN;
  row.ridgeSlopeHzPerSecPerDeg = NaN;
  if isfinite(row.bestDoaOffsetDeg) && abs(row.bestDoaOffsetDeg) > eps
    if surfaceKindNow == "doa-fdref"
      row.ridgeSlopeHzPerDeg = row.bestFdRefOffsetHz / row.bestDoaOffsetDeg;
    elseif surfaceKindNow == "doa-fdrate"
      row.ridgeSlopeHzPerSecPerDeg = row.bestFdRateOffsetHzPerSec / row.bestDoaOffsetDeg;
    end
  end
  row.boundaryHit = localIsRidgeBoundaryHit(bestRow, surfaceKindNow, config);
  rowCell{iKey} = row;
end
ridgeSummaryTable = struct2table([rowCell{:}].');
end

function centerLineSummaryTable = localBuildCenterLineSummaryTable(candidateProbeTable)
%LOCALBUILDCENTERLINESUMMARYTABLE Summarize final-to-center line traces.

if isempty(candidateProbeTable) || ~ismember('surfaceKind', candidateProbeTable.Properties.VariableNames)
  centerLineSummaryTable = table();
  return;
end
lineMask = string(candidateProbeTable.surfaceKind) == "center-line";
if ~any(lineMask)
  centerLineSummaryTable = table();
  return;
end
lineTable = candidateProbeTable(lineMask, :);
keyTable = unique(lineTable(:, {'taskSeed', 'ridgeDirectionFamily'}), 'rows', 'stable');
rowCell = cell(height(keyTable), 1);
for iKey = 1:height(keyTable)
  seedNow = keyTable.taskSeed(iKey);
  directionNow = string(keyTable.ridgeDirectionFamily(iKey));
  seedLine = lineTable(lineTable.taskSeed == seedNow & string(lineTable.ridgeDirectionFamily) == directionNow, :);
  seedLine = seedLine(isfinite(seedLine.obj), :);
  [~, bestIdx] = min(seedLine.obj);
  bestRow = seedLine(bestIdx, :);
  terminalMask = abs(seedLine.lineAlpha - 1) < 1e-12;
  terminalRow = seedLine([], :);
  if any(terminalMask)
    terminalRow = seedLine(find(terminalMask, 1, 'first'), :);
  end
  defaultRow = localBestProbeRow(candidateProbeTable(candidateProbeTable.taskSeed == seedNow, :), "default-final");
  row = struct();
  row.taskSeed = seedNow;
  row.ridgeDirectionFamily = directionNow;
  row.truthFreeDirection = directionNow ~= "truth";
  row.bestLineAlpha = localProbeValue(bestRow, 'lineAlpha', NaN);
  row.bestObjGainFromDefault = localProbeValue(bestRow, 'objGainFromDefault', NaN);
  row.bestAngleErrDeg = localProbeValue(bestRow, 'angleErrDeg', NaN);
  row.defaultAngleErrDeg = localProbeValue(defaultRow, 'angleErrDeg', NaN);
  row.bestAngleGainFromDefaultDeg = row.defaultAngleErrDeg - row.bestAngleErrDeg;
  ...
  row.bestFdRefOffsetHz = localProbeValue(bestRow, 'fdRefStepHz', NaN);
  row.bestFdRateOffsetHzPerSec = localProbeValue(bestRow, 'fdRateStepHzPerSec', NaN);
  row.terminalObjGainFromDefault = localProbeValue(terminalRow, 'objGainFromDefault', NaN);
  row.terminalAngleErrDeg = localProbeValue(terminalRow, 'angleErrDeg', NaN);
  row.terminalNonRefCoherenceFloor = localProbeValue(terminalRow, 'nonRefCoherenceFloor', NaN);
  rowCell{iKey} = row;
end
centerLineSummaryTable = struct2table([rowCell{:}].');
end

function tf = localIsRidgeBoundaryHit(bestRow, surfaceKind, config)
%LOCALISRIDGEBOUNDARYHIT Check whether a ridge minimum is on the scan edge.

tf = false;
if isempty(bestRow)
  return;
end
doaOffsetList = reshape(localGetFieldOrDefault(config, 'ridgeDoaOffsetDegList', []), [], 1);
if localIsValueAtGridEdge(localProbeValue(bestRow, 'ridgeDoaOffsetDeg', NaN), doaOffsetList)
  tf = true;
  return;
end
if string(surfaceKind) == "doa-fdref"
  fdList = reshape(localGetFieldOrDefault(config, 'ridgeFdRefOffsetHzList', []), [], 1);
  tf = localIsValueAtGridEdge(localProbeValue(bestRow, 'fdRefStepHz', NaN), fdList);
elseif string(surfaceKind) == "doa-fdrate"
  rateList = reshape(localGetFieldOrDefault(config, 'ridgeFdRateOffsetHzPerSecList', []), [], 1);
  tf = localIsValueAtGridEdge(localProbeValue(bestRow, 'fdRateStepHzPerSec', NaN), rateList);
end
end

function tf = localIsValueAtGridEdge(value, gridVec)
%LOCALISVALUEATGRIDEDGE Return true when a value equals either grid boundary.

tf = false;
gridVec = reshape(double(gridVec), [], 1);
gridVec = gridVec(isfinite(gridVec));
if ~(isfinite(value) && ~isempty(gridVec))
  return;
end
gridMin = min(gridVec);
gridMax = max(gridVec);
tol = max(1e-12, 1e-9 * max(1, max(abs(gridVec))));
tf = abs(value - gridMin) <= tol || abs(value - gridMax) <= tol;
end
...
function candidateCoverageTable = localBuildCandidateCoverageTable(candidateWinnerTable, config)
%LOCALBUILDCANDIDATECOVERAGETABLE Convert winner rows into DoA-hit coverage diagnostics.

candidateCoverageTable = table();
if isempty(candidateWinnerTable)
  return;
end
angleHitThreshold = localGetFieldOrDefault(config, 'angleHitThresholdDeg', 0.002);
numRow = height(candidateWinnerTable);
rowCell = cell(numRow, 1);
for iRow = 1:numRow
  src = candidateWinnerTable(iRow, :);
  defaultHit = localIsAngleHit(src.defaultAngleErrDeg, angleHitThreshold);
  pureHit = localIsAngleHit(src.bestPureDoaAngleErrDeg, angleHitThreshold);
  jointHit = localIsAngleHit(src.bestJointAngleErrDeg, angleHitThreshold);
  wideHit = localIsAngleHit(src.bestWideCenteredAngleErrDeg, angleHitThreshold);
  implementableHit = localIsAngleHit(src.bestImplementableCenterAngleErrDeg, angleHitThreshold);
  truthHit = localIsAngleHit(src.truthDoaOracleAngleErrDeg, angleHitThreshold);
  row = struct();
  row.taskSeed = src.taskSeed(1);
  row.tailClass = string(src.tailClass(1));
  row.candidateClass = string(src.candidateClass(1));
  row.angleHitThresholdDeg = angleHitThreshold;
  row.defaultHit = defaultHit;
  row.pureDoaHit = pureHit;
  row.jointDoaFdRefHit = jointHit;
  row.wideCenteredHit = wideHit;
  row.implementableCenterHit = implementableHit;
  row.truthOracleHit = truthHit;
  row.implementableCoversTruth = implementableHit || wideHit || pureHit;
  row.jointAddsOverPure = jointHit && ~pureHit;
  row.jointUsesNonzeroFdRef = isfinite(src.bestJointFdRefStepHz(1)) && abs(src.bestJointFdRefStepHz(1)) > 1e-9;
  row.bestImplementableCenterFamily = string(src.bestImplementableCenterFamily(1));
  row.bestImplementableAngleErrDeg = src.bestImplementableCenterAngleErrDeg(1);
  row.bestImplementableNonRefCoherenceFloor = src.bestImplementableCenterNonRefCoherenceFloor(1);
  row.bestJointFdRefStepHz = src.bestJointFdRefStepHz(1);
  rowCell{iRow} = row;
end
candidateCoverageTable = struct2table([rowCell{:}].');
end

function policyRecommendationTable = localBuildPolicyRecommendationTable(candidateProbeTable, tailDiagnosisTable, config)
%LOCALBUILDPOLICYRECOMMENDATIONTABLE Offline-simulate gate/adoption policies on cached probes.

policyRecommendationTable = table();
if isempty(candidateProbeTable)
  return;
end
angleHitThreshold = localGetFieldOrDefault(config, 'angleHitThresholdDeg', 0.002);
gateThresholdList = [0.20; 0.22; 0.25; 0.30];
familyStepList = [0.004; 0.006; 0.008];
safeCoherenceList = [0.90; 0.95; 0.98];
seedList = unique(candidateProbeTable.taskSeed, 'stable');
rowCell = cell(numel(gateThresholdList) * numel(familyStepList) * numel(safeCoherenceList), 1);
rowCount = 0;
for iGate = 1:numel(gateThresholdList)
  for iStep = 1:numel(familyStepList)
    for iCoh = 1:numel(safeCoherenceList)
      gateThreshold = gateThresholdList(iGate);
      familyStep = familyStepList(iStep);
      safeCoherence = safeCoherenceList(iCoh);
      selectedAngle = nan(numel(seedList), 1);
      defaultAngle = nan(numel(seedList), 1);
      selectedRateVec = false(numel(seedList), 1);
      hardMask = false(numel(seedList), 1);
      easyMask = false(numel(seedList), 1);
      fdMask = false(numel(seedList), 1);
      damagedVec = false(numel(seedList), 1);
      triggerVec = false(numel(seedList), 1);
      for iSeed = 1:numel(seedList)
        ...
        defaultAngle(iSeed) = localProbeValue(defaultRow, 'angleErrDeg', NaN);
        tailRow = tailDiagnosisTable(tailDiagnosisTable.taskSeed == seedNow, :);
        caseRole = localClassifyRescueCaseRole(string(localTableValue(tailRow, 'tailClass', "")));
        hardMask(iSeed) = caseRole == "hard-collapse";
        easyMask(iSeed) = caseRole == "easy-negative";
        fdMask(iSeed) = caseRole == "fd-not-healthy-negative";
        triggerVec(iSeed) = isfinite(defaultCoherence) && defaultCoherence < gateThreshold;
        if triggerVec(iSeed)
          selectedRow = localSelectPolicyRow(seedProbe, familyStep, safeCoherence, 300);
          selectedRateVec(iSeed) = string(localProbeValue(selectedRow, 'candidateFamily', "")) ~= "default-final";
        end
        selectedAngle(iSeed) = localProbeValue(selectedRow, 'angleErrDeg', defaultAngle(iSeed));
        damagedVec(iSeed) = isfinite(defaultAngle(iSeed)) && isfinite(selectedAngle(iSeed)) && ...
          selectedAngle(iSeed) > defaultAngle(iSeed) + localGetFieldOrDefault(config, 'probeDamageThresholdDeg', 5e-4);
      end
      rowCount = rowCount + 1;
      row = struct();
      row.policyName = string(sprintf('gate%.2f_step%.3f_coh%.2f', gateThreshold, familyStep, safeCoherence));
      row.gateCoherenceThreshold = gateThreshold;
      row.familyMaxAbsDoaStepDeg = familyStep;
      row.safeCoherenceThreshold = safeCoherence;
      row.numCase = numel(seedList);
      row.numHardCase = nnz(hardMask);
      row.triggerRate = localMeanLogical(triggerVec);
      row.hardTriggerRate = localMeanLogical(triggerVec(hardMask));
      row.selectedRate = localMeanLogical(selectedRateVec);
      row.angleHitThresholdDeg = angleHitThreshold;
      row.defaultAngleHitRate = localMeanAngleHit(defaultAngle, angleHitThreshold);
      row.selectedAngleHitRate = localMeanAngleHit(selectedAngle, angleHitThreshold);
      row.hardAngleHitRate = localMeanAngleHit(selectedAngle(hardMask), angleHitThreshold);
      row.overallP95AngleDeg = localPercentile(selectedAngle, 95);
      row.overallP99AngleDeg = localPercentile(selectedAngle, 99);
      row.overallMaxAngleDeg = max(selectedAngle, [], 'omitnan');
      row.easyDamageRate = localMeanLogical(damagedVec(easyMask));
      row.fdNegativeDamageRate = localMeanLogical(damagedVec(fdMask));
      row.recommendation = localRecommendPolicyRow(row);
      rowCell{rowCount} = row;
    end
  end
end
rowCell = rowCell(1:rowCount);
policyRecommendationTable = struct2table([rowCell{:}].');
end

function selectedRow = localSelectPolicyRow(seedProbe, familyStep, safeCoherence, maxAbsFdRefStepHz)
%LOCALSELECTPOLICYROW Select the best implementable row under a replay-only policy.

familyList = ["default-final"; "wide-coarse-doa-grid"; "single-mf-coarse-doa-grid"];
selectedRow = localBestProbeRow(seedProbe, "default-final");
mask = ismember(seedProbe.candidateFamily, familyList) & isfinite(seedProbe.obj);
idx = find(mask);
allowed = false(numel(idx), 1);
for iIdx = 1:numel(idx)
  rowNow = seedProbe(idx(iIdx), :);
  familyNow = string(localProbeValue(rowNow, 'candidateFamily', ""));
  if familyNow == "default-final"
    allowed(iIdx) = true;
    continue;
  end
  objGain = localProbeValue(rowNow, 'objGainFromDefault', NaN);
  coherenceFloor = localProbeValue(rowNow, 'nonRefCoherenceFloor', NaN);
  doaStep = [localProbeValue(rowNow, 'doaStep1Deg', NaN), localProbeValue(rowNow, 'doaStep2Deg', NaN)];
  fdRefStep = localProbeValue(rowNow, 'fdRefStepHz', NaN);
  allowed(iIdx) = isfinite(objGain) && objGain > 0 && ...
    isfinite(coherenceFloor) && coherenceFloor >= safeCoherence && ...
    all(isfinite(doaStep)) && max(abs(doaStep)) <= familyStep && ...
    isfinite(fdRefStep) && abs(fdRefStep) <= maxAbsFdRefStepHz;
end
if any(allowed)
  idxAllowed = idx(allowed);
  [~, idxRel] = min(seedProbe.obj(idxAllowed));
  selectedRow = seedProbe(idxAllowed(idxRel), :);
end
end

function tf = localIsAngleHit(angleErrDeg, threshold)
%LOCALISANGLEHIT Return true when one angle error is within the hit threshold.

tf = isfinite(angleErrDeg) && isfinite(threshold) && angleErrDeg <= threshold;
end

function value = localMeanAngleHit(angleErrDeg, threshold)
%LOCALMEANANGLEHIT Return the fraction of angle errors within threshold.

if isempty(angleErrDeg) || ~(isfinite(threshold) && threshold >= 0)
  value = NaN;
else
  value = mean(double(angleErrDeg <= threshold), 'omitnan');
end
end

function recommendation = localRecommendPolicyRow(row)
%LOCALRECOMMENDPOLICYROW Label a safe policy sweep row.

if row.easyDamageRate > 0 || row.fdNegativeDamageRate > 0
  recommendation = "hold-damage";
elseif row.overallMaxAngleDeg > 0.006
  recommendation = "hold-max-tail";
elseif row.selectedAngleHitRate >= 0.99
  recommendation = "candidate-99-hit";
elseif row.hardAngleHitRate >= 0.8
  recommendation = "candidate-hard-hit";
else
...
%LOCALPLOTREPLAY Draw compact ridge surface diagnostics.

if isempty(plotData) || ~isstruct(plotData)
  return;
end
candidateProbeTable = localGetFieldOrDefault(plotData, 'candidateProbeTable', table());
if localPlotRidgeSurfacePreview(candidateProbeTable)
  return;
end
...
function didPlot = localPlotRidgeSurfacePreview(candidateProbeTable)
%LOCALPLOTRIDGESURFACEPREVIEW Plot one representative DoA-fdRef and DoA-fdRate ridge surface.

didPlot = false;
if isempty(candidateProbeTable) || ~ismember('surfaceKind', candidateProbeTable.Properties.VariableNames)
  return;
end
mask = ismember(string(candidateProbeTable.surfaceKind), ["doa-fdref"; "doa-fdrate"]);
if ~any(mask)
  return;
end
surfaceTable = candidateProbeTable(mask, :);
truthFreeMask = string(surfaceTable.ridgeDirectionFamily) ~= "truth";
if any(truthFreeMask)
  surfaceTable = surfaceTable(truthFreeMask, :);
end
seedUse = surfaceTable.taskSeed(1);
directionUse = string(surfaceTable.ridgeDirectionFamily(1));
plotTable = surfaceTable(surfaceTable.taskSeed == seedUse & string(surfaceTable.ridgeDirectionFamily) == directionUse, :);
fdRefTable = plotTable(string(plotTable.surfaceKind) == "doa-fdref", :);
fdRateTable = plotTable(string(plotTable.surfaceKind) == "doa-fdrate", :);
if isempty(fdRefTable) && isempty(fdRateTable)
  return;
end
figure('Name', 'In-tooth DoA-Doppler ridge trace');
tiledlayout(2, 1, 'TileSpacing', 'compact');
if ~isempty(fdRefTable)
  nexttile;
  localPlotOneSurface(fdRefTable, 'fdRefStepHz', 'fdRef offset (Hz)', ...
    sprintf('seed %d, direction %s: DoA-fdRef objective gain', seedUse, char(directionUse)));
end
if ~isempty(fdRateTable)
  nexttile;
  localPlotOneSurface(fdRateTable, 'fdRateStepHzPerSec', 'fdRate offset (Hz/s)', ...
    sprintf('seed %d, direction %s: DoA-fdRate objective gain', seedUse, char(directionUse)));
end
didPlot = true;
end

function localPlotOneSurface(surfaceTable, xField, xLabelText, titleText)
%LOCALPLOTONESURFACE Draw one objective-gain heatmap from a surface table.

xGrid = unique(surfaceTable.(xField), 'stable');
yGrid = unique(surfaceTable.ridgeDoaOffsetDeg, 'stable');
zMat = nan(numel(yGrid), numel(xGrid));
for iRow = 1:height(surfaceTable)
  xIdx = find(xGrid == surfaceTable.(xField)(iRow), 1, 'first');
  yIdx = find(yGrid == surfaceTable.ridgeDoaOffsetDeg(iRow), 1, 'first');
  if ~isempty(xIdx) && ~isempty(yIdx)
    zMat(yIdx, xIdx) = surfaceTable.objGainFromDefault(iRow);
  end
end
imagesc(xGrid, yGrid, zMat);
set(gca, 'YDir', 'normal');
colorbar;
xlabel(xLabelText);
ylabel('DoA offset along direction (deg)');
...
if ~(isfinite(toothStepHz) && toothStepHz > 0)
  error('replayMfInToothDoaFdRangeEnvelope:MissingToothStep', ...
