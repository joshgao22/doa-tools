...
% Purpose: diagnose truth-free adoption gates for wide-centered basin-entry
% candidates on ordinary in-tooth DoA misses. The replay assumes the fdRef tooth
% is already correct by using a truth-centered half-tooth fd range, then labels
% ordinary/easy/collapse/fd-negative cases offline while sweeping gates that use
% only objective gain, coherence recovery, DoA step, and fd-step diagnostics.
%
...
replayName = "replayMfInToothOrdinaryWideGateDiagnose";
...
includeFamilySafeAdoptBank = false;   % false bypasses hard-collapse family-safe tables in this ordinary-wide replay.
includeLegacyFailedBanks = false;    % false bypasses one-center/blanket banks already shown unstable.
includeFinalSmallPolishProbe = false; % false bypasses the already-negative final-centered small-polish grid.
includeSingleMfBasinEntryProbe = false; % false bypasses the already-negative single-MF ordinary basin-entry grid.
includeHardCollapseAuxTables = false; % false keeps this replay focused on ordinary-wide gate validation.
...
wideGateObjGainThresholdList = [0; 5; 10; 20; 30; 50; 80; 100; 150; 200]; % Fine no-truth objective-gain sweep for ordinary wide adoption.
wideGateMinAbsDoaStepDegList = [0; 0.001; 0.0015; 0.002]; % No-truth default-vs-wide DoA-disagreement lower-bound sweep.
wideGateMaxAbsDoaStepDegList = [0.004; 0.006]; % No-truth wide adoption DoA-step guard sweep.
wideGateCandidateCoherenceThreshold = 0.95; % No-truth wide adoption coherence recovery threshold.
wideGateRequireDefaultNonCollapse = true; % true limits ordinary-wide tests to non-collapse default cases.
wideGateMaxAbsFdRateStepHzPerSec = 50; % No-truth wide adoption fdRate-step guard.
wideGatePreferredObjGain = 10; % Current replay-level preferred ordinary-wide objective gain threshold.
wideGatePreferredMinAbsDoaStepDeg = 0.001; % Current preferred lower guard for default-vs-wide DoA disagreement.
wideGatePreferredMaxAbsDoaStepDeg = 0.004; % Current preferred upper guard for wide-only ordinary adoption.
wideGateStrictEasyTriggerRateMax = 0.10; % Candidate gate should keep easy triggers low before flow promotion.
...
config.includeFinalSmallPolishProbe = includeFinalSmallPolishProbe;
config.includeSingleMfBasinEntryProbe = includeSingleMfBasinEntryProbe;
config.includeHardCollapseAuxTables = includeHardCollapseAuxTables;
...
config.wideGateObjGainThresholdList = wideGateObjGainThresholdList(:);
config.wideGateMinAbsDoaStepDegList = wideGateMinAbsDoaStepDegList(:);
config.wideGateMaxAbsDoaStepDegList = wideGateMaxAbsDoaStepDegList(:);
config.wideGateCandidateCoherenceThreshold = wideGateCandidateCoherenceThreshold;
config.wideGateRequireDefaultNonCollapse = wideGateRequireDefaultNonCollapse;
config.wideGateMaxAbsFdRateStepHzPerSec = wideGateMaxAbsFdRateStepHzPerSec;
config.wideGatePreferredObjGain = wideGatePreferredObjGain;
config.wideGatePreferredMinAbsDoaStepDeg = wideGatePreferredMinAbsDoaStepDeg;
config.wideGatePreferredMaxAbsDoaStepDeg = wideGatePreferredMaxAbsDoaStepDeg;
config.wideGateStrictEasyTriggerRateMax = wideGateStrictEasyTriggerRateMax;
...
localPrintWideGateReplayConfig(config);
...
try
  if config.checkpointEnable
    ...
    if ~runState.isComplete
      error('replayMfInToothOrdinaryWideGateDiagnose:CheckpointIncomplete', ...
...
wideGateFeatureTable = localBuildWideGateFeatureTable(ordinaryCandidateTable, config);
wideGateDecisionTable = localBuildWideGateDecisionTable(ordinarySeedTable, ordinaryCandidateTable, config);
wideGatePolicyTable = localBuildWideGatePolicyTable(wideGateDecisionTable, config);
wideGateRecommendedPolicyTable = localSelectWideGateRecommendedPolicy(wideGatePolicyTable, config);
if localGetLogicalField(config, 'includeHardCollapseAuxTables', false)
...
plotData.wideGateFeatureTable = wideGateFeatureTable;
plotData.wideGateDecisionTable = wideGateDecisionTable;
plotData.wideGatePolicyTable = wideGatePolicyTable;
plotData.wideGateRecommendedPolicyTable = wideGateRecommendedPolicyTable;
...
replayData.wideGateFeatureTable = wideGateFeatureTable;
replayData.wideGateDecisionTable = wideGateDecisionTable;
replayData.wideGatePolicyTable = wideGatePolicyTable;
replayData.wideGateRecommendedPolicyTable = wideGateRecommendedPolicyTable;
...
notifyMfReplayStatus(struct( ...
  ...
  'commentLineList', "Ordinary-wide gate diagnose completed."));
catch ME
...
fprintf('Running replayMfInToothOrdinaryWideGateDiagnose ...\n');
fprintf('  repeats                         : %d\n', replayData.config.numRepeat);
...
if isfield(replayData, 'wideGateRecommendedPolicyTable') && ~isempty(replayData.wideGateRecommendedPolicyTable)
  fprintf('\n========== Ordinary-wide recommended policy ==========\n');
  disp(replayData.wideGateRecommendedPolicyTable);
end
if isfield(replayData, 'wideGatePolicyTable') && ~isempty(replayData.wideGatePolicyTable)
  fprintf('\n========== Ordinary-wide gate policy sweep ==========\n');
  disp(replayData.wideGatePolicyTable);
end
if isfield(replayData, 'wideGateFeatureTable') && ~isempty(replayData.wideGateFeatureTable)
  fprintf('\n========== Wide candidate feature table ==========\n');
  disp(replayData.wideGateFeatureTable);
end
...
if isfield(replayData, 'wideGateDecisionTable')
  fprintf('\n========== Ordinary-wide gate decision table ==========\n');
  localDispTablePreview(replayData.wideGateDecisionTable, 16);
end
...
if isfield(replayData, 'rescueEffectAggregateTable') && ~isempty(replayData.rescueEffectAggregateTable)
...
if isfield(replayData, 'triggerReasonTable') && ~isempty(replayData.triggerReasonTable)
  fprintf('\n========== Trigger reason table ==========\n');
  localDispTablePreview(replayData.triggerReasonTable, 8);
end
if isfield(replayData, 'rescueBankDecisionTable') && ~isempty(replayData.rescueBankDecisionTable)
  fprintf('\n========== Rescue bank decision table ==========\n');
  localDispTablePreview(replayData.rescueBankDecisionTable, 8);
end
if isfield(replayData, 'rescueEffectCaseTable') && ~isempty(replayData.rescueEffectCaseTable)
...
localPlotWideGateReplay(replayData.plotData);

%% Local helpers

function localPrintWideGateReplayConfig(config)
%LOCALPRINTWIDEGATEREPLAYCONFIG Print replay-specific settings after the common header.
...
fprintf('  %-32s : %s deg\n', 'basin-entry DoA steps', mat2str(config.coarseDoaStepDegList(:).'));
if localGetLogicalField(config, 'includeFinalSmallPolishProbe', false)
  fprintf('  %-32s : %s deg\n', 'final polish DoA steps', mat2str(config.finalPolishDoaStepDegList(:).'));
else
  fprintf('  %-32s : bypassed\n', 'final polish probe');
end
fprintf('  %-32s : %d\n', 'single-MF basin probe', config.includeSingleMfBasinEntryProbe);
fprintf('  %-32s : %d\n', 'hard-collapse aux tables', config.includeHardCollapseAuxTables);
...
fprintf('  %-32s : %.3f\n', 'ordinary coherence threshold', config.ordinaryCoherenceMinThreshold);
fprintf('  %-32s : %s\n', 'wide gate obj-gain sweep', mat2str(config.wideGateObjGainThresholdList(:).'));
fprintf('  %-32s : %s deg\n', 'wide gate min DoA-step sweep', mat2str(config.wideGateMinAbsDoaStepDegList(:).'));
fprintf('  %-32s : %s deg\n', 'wide gate max DoA-step sweep', mat2str(config.wideGateMaxAbsDoaStepDegList(:).'));
fprintf('  %-32s : %.3f\n', 'wide gate candidate coherence', config.wideGateCandidateCoherenceThreshold);
fprintf('  %-32s : %d\n', 'wide gate require non-collapse', config.wideGateRequireDefaultNonCollapse);
fprintf('  %-32s : obj>=%.4g, %.4g<=step<=%.4g deg\n', 'wide gate preferred policy', ...
  config.wideGatePreferredObjGain, config.wideGatePreferredMinAbsDoaStepDeg, ...
  config.wideGatePreferredMaxAbsDoaStepDeg);
fprintf('  %-32s : %.4f deg\n', 'angle hit threshold', config.angleHitThresholdDeg);
end

function tf = localGetLogicalField(dataStruct, fieldName, defaultValue)
%LOCALGETLOGICALFIELD Read one logical scalar field with a default.

tf = logical(defaultValue);
if isstruct(dataStruct) && isfield(dataStruct, fieldName) && ~isempty(dataStruct.(fieldName))
  tf = logical(dataStruct.(fieldName));
end
end
...
checkpointOpt.meta = struct( ...
  ...
  'includeFinalSmallPolishProbe', config.includeFinalSmallPolishProbe, ...
  'includeSingleMfBasinEntryProbe', config.includeSingleMfBasinEntryProbe, ...
  'includeHardCollapseAuxTables', config.includeHardCollapseAuxTables, ...
  'wideGateObjGainThresholdList', reshape(config.wideGateObjGainThresholdList, 1, []), ...
  'wideGateMinAbsDoaStepDegList', reshape(config.wideGateMinAbsDoaStepDegList, 1, []), ...
  'wideGateMaxAbsDoaStepDegList', reshape(config.wideGateMaxAbsDoaStepDegList, 1, []), ...
  'wideGateCandidateCoherenceThreshold', config.wideGateCandidateCoherenceThreshold, ...
  'wideGateRequireDefaultNonCollapse', config.wideGateRequireDefaultNonCollapse, ...
...
%LOCALBUILDCHECKPOINTRUNKEY Build a compact checkpoint key for this replay.
...
runKey = string(sprintf('seed%dto%d_rep%d_snr%.2f_owide_%s', ...
  seedList(1), seedList(end), numel(seedList), config.snrDb, signatureHash));
runKey = replace(runKey, '.', 'p');
runKey = replace(runKey, '-', 'm');
end

function signature = localBuildCheckpointSignature(config)
%LOCALBUILDCHECKPOINTSIGNATURE Build a full semantic signature for run-key hashing.

if config.gatedRescueGateVersion == "coherence-v1"
  gatePart = sprintf('%s_coh%.6g', char(config.gatedRescueGateVersion), ...
    config.gatedRescueCoherenceThreshold);
else
  gatePart = sprintf('%s_coh%.6g_phase%.6g', char(config.gatedRescueGateVersion), ...
    config.gatedRescueCoherenceThreshold, config.gatedRescuePhaseResidThresholdRad);
end
signature = strjoin(string({ ...
  gatePart, ...
  sprintf('ordinaryCoh%.6g', config.ordinaryCoherenceMinThreshold), ...
  sprintf('ordinaryTruthGap%.6g', config.ordinaryTruthGapThresholdDeg), ...
  sprintf('angleHit%.6g', config.angleHitThresholdDeg), ...
  sprintf('finalPolish%d', config.includeFinalSmallPolishProbe), ...
  sprintf('singleMf%d', config.includeSingleMfBasinEntryProbe), ...
  sprintf('hardAux%d', config.includeHardCollapseAuxTables), ...
  sprintf('safeDoa%.6g', config.safeAdoptMaxAbsDoaStepDeg), ...
  sprintf('familySafeDoa%.6g', config.familySafeAdoptMaxAbsDoaStepDeg), ...
  sprintf('safeCoh%.6g', config.safeAdoptCoherenceThreshold), ...
  sprintf('safeObj%.6g', config.safeAdoptMinObjGain), ...
  sprintf('safeFd%.6g', config.safeAdoptMaxAbsFdRefStepHz), ...
  sprintf('joint%d', config.includeJointSafeAdoptBank), ...
  sprintf('family%d', config.includeFamilySafeAdoptBank), ...
  sprintf('legacy%d', config.includeLegacyFailedBanks), ...
  sprintf('jointDoa%s', mat2str(reshape(config.jointDoaStepDegList, 1, []), 8)), ...
  sprintf('jointFd%s', mat2str(reshape(config.jointFdRefOffsetHzList, 1, []), 8)), ...
  sprintf('wideObj%s', mat2str(reshape(config.wideGateObjGainThresholdList, 1, []), 8)), ...
  sprintf('wideMin%s', mat2str(reshape(config.wideGateMinAbsDoaStepDegList, 1, []), 8)), ...
  sprintf('wideMax%s', mat2str(reshape(config.wideGateMaxAbsDoaStepDegList, 1, []), 8)), ...
  sprintf('wideCoh%.6g', config.wideGateCandidateCoherenceThreshold), ...
  sprintf('wideNonCol%d', config.wideGateRequireDefaultNonCollapse), ...
  sprintf('oracleFdFrac%.6g', config.oracleFdHalfToothFraction), ...
  sprintf('oracleFdRate%.6g', config.oracleFdRateHalfWidthHzPerSec) ...
  }), '|');
end
...
if ~(isfinite(truthFdRefHz) && isfinite(truthFdRateHzPerSec))
  error('replayMfInToothOrdinaryWideGateDiagnose:MissingTruthFdLine', ...
...
switch method.initMode
  ...
  otherwise
    error('replayMfInToothOrdinaryWideGateDiagnose:UnknownInitMode', ...
...
switch method.viewMode
  ...
  otherwise
    error('replayMfInToothOrdinaryWideGateDiagnose:UnknownViewMode', ...
...
switch method.doaSeedMode
  ...
  otherwise
    error('replayMfInToothOrdinaryWideGateDiagnose:UnknownDoaSeedMode', ...
...
if localGetLogicalField(config, 'includeSingleMfBasinEntryProbe', false) && ...
    ~isempty(singleIdx) && singleIdx <= numel(methodCaseCell)
...
function familyList = localBuildOrdinaryCandidateFamilyList(config)
%LOCALBUILDORDINARYCANDIDATEFAMILYLIST Return displayed candidate families for enabled probes.

familyList = ["default-final", "default"; "wide-coarse-doa-grid", "wide-basin-entry"];
if localGetLogicalField(config, 'includeFinalSmallPolishProbe', false)
  familyList = [familyList; "final-small-doa-grid", "final-small-polish"]; %#ok<AGROW>
end
if localGetLogicalField(config, 'includeSingleMfBasinEntryProbe', false)
  familyList = [familyList; "single-mf-coarse-doa-grid", "single-mf-basin-entry"]; %#ok<AGROW>
end
...
familyList = localBuildOrdinaryCandidateFamilyList(config);
...
function featureTable = localBuildWideGateFeatureTable(ordinaryCandidateTable, config)
%LOCALBUILDWIDEGATEFEATURETABLE Summarize no-truth wide-candidate features by offline label.

if isempty(ordinaryCandidateTable)
  featureTable = table();
  return;
end
wideData = ordinaryCandidateTable(ordinaryCandidateTable.candidateGroup == "wide-basin-entry", :);
if isempty(wideData)
  featureTable = table();
  return;
end
groupList = unique(wideData.missType, 'stable');
rowCell = cell(numel(groupList), 1);
for iGroup = 1:numel(groupList)
  missTypeNow = groupList(iGroup);
  mask = wideData.missType == missTypeNow;
  maxAbsDoaStep = max(abs([wideData.candidateDoaStep1Deg(mask), wideData.candidateDoaStep2Deg(mask)]), [], 2);
  row = struct();
  row.missType = missTypeNow;
  row.numCase = nnz(mask);
  row.defaultHitRate = localMeanHit(wideData.defaultAngleErrDeg(mask), config.angleHitThresholdDeg);
  row.wideHitRate = localMeanHit(wideData.candidateAngleErrDeg(mask), config.angleHitThresholdDeg);
  row.wideRescueRate = localMeanOptionalLogical(wideData.isRescued(mask));
  row.wideDamageRate = localMeanOptionalLogical(wideData.isDamaged(mask));
  row.medianObjGain = median(wideData.candidateObjGainFromDefault(mask), 'omitnan');
  row.p95ObjGain = localPercentile(wideData.candidateObjGainFromDefault(mask), 95);
  row.medianAngleGainDeg = median(wideData.angleGainDeg(mask), 'omitnan');
  row.medianWideAngleDeg = median(wideData.candidateAngleErrDeg(mask), 'omitnan');
  row.p95WideAngleDeg = localPercentile(wideData.candidateAngleErrDeg(mask), 95);
  row.medianDefaultCoherence = median(wideData.defaultNonRefCoherenceFloor(mask), 'omitnan');
  row.medianWideCoherence = median(wideData.candidateNonRefCoherenceFloor(mask), 'omitnan');
  row.medianMaxAbsDoaStepDeg = median(maxAbsDoaStep, 'omitnan');
  row.p05MaxAbsDoaStepDeg = localPercentile(maxAbsDoaStep, 5);
  row.p95MaxAbsDoaStepDeg = localPercentile(maxAbsDoaStep, 95);
  rowCell{iGroup} = row;
end
featureTable = struct2table([rowCell{:}].');
end

function decisionTable = localBuildWideGateDecisionTable(ordinarySeedTable, ordinaryCandidateTable, config)
%LOCALBUILDWIDEGATEDECISIONTABLE Apply truth-free wide gates and evaluate them offline.

if isempty(ordinarySeedTable) || isempty(ordinaryCandidateTable)
  decisionTable = table();
  return;
end
policySpec = localBuildWideGatePolicySpec(config);
rowCell = cell(height(ordinarySeedTable) * numel(policySpec), 1);
rowCount = 0;
for iSeed = 1:height(ordinarySeedTable)
  seedRow = ordinarySeedTable(iSeed, :);
  taskSeed = seedRow.taskSeed;
  seedCandidates = ordinaryCandidateTable(ordinaryCandidateTable.taskSeed == taskSeed, :);
  defaultRow = seedCandidates(seedCandidates.candidateGroup == "default", :);
  wideRow = seedCandidates(seedCandidates.candidateGroup == "wide-basin-entry", :);
  if isempty(defaultRow)
    continue;
  end
  if ~isempty(wideRow)
    wideRow = wideRow(1, :);
  ...
  for iPolicy = 1:numel(policySpec)
    policy = policySpec(iPolicy);
    [useWide, gateReason] = localEvaluateWideGatePolicy(defaultRow, wideRow, policy, config);
    selectedRow = defaultRow;
    if useWide && ~isempty(wideRow)
      selectedRow = wideRow;
    end
    defaultAngle = localTableScalar(defaultRow, 'candidateAngleErrDeg', NaN);
    selectedAngle = localTableScalar(selectedRow, 'candidateAngleErrDeg', NaN);
    row = struct();
    row.taskSeed = taskSeed;
    row.policyLabel = policy.label;
    row.policyFamily = policy.family;
    row.minObjGain = policy.minObjGain;
    row.minAbsDoaStepDeg = policy.minAbsDoaStepDeg;
    row.maxAbsDoaStepDeg = policy.maxAbsDoaStepDeg;
    row.requireDefaultNonCollapse = policy.requireDefaultNonCollapse;
    row.missType = seedRow.missType;
    row.isOrdinaryAngleMiss = logical(seedRow.isOrdinaryAngleMiss);
    row.isAngleMiss = logical(seedRow.isAngleMiss);
    row.isEasyHit = seedRow.missType == "easy-hit";
    row.isCollapseHard = seedRow.missType == "collapse-hard";
    row.isFdNegative = startsWith(seedRow.missType, "fd-not-healthy");
    row.defaultAngleErrDeg = defaultAngle;
    row.wideAngleErrDeg = localTableScalar(wideRow, 'candidateAngleErrDeg', NaN);
    row.selectedAngleErrDeg = selectedAngle;
    row.selectedCandidateGroup = localTableScalar(selectedRow, 'candidateGroup', "");
    row.selectedCandidateFamily = localTableScalar(selectedRow, 'selectedCandidateFamily', "");
    row.selectedCandidateTag = localTableScalar(selectedRow, 'selectedCandidateTag', "");
    row.selectedSourceMethodLabel = localTableScalar(selectedRow, 'selectedSourceMethodLabel', "");
    row.gateTriggered = logical(useWide);
    row.gateReason = string(gateReason);
    row.angleGainDeg = defaultAngle - selectedAngle;
    row.isHit = selectedAngle <= config.angleHitThresholdDeg;
    row.isRescued = defaultAngle > config.angleHitThresholdDeg && selectedAngle <= config.angleHitThresholdDeg;
    row.isDamaged = selectedAngle > defaultAngle + config.damageThresholdDeg;
    row.ordinaryRescued = row.isOrdinaryAngleMiss && row.isRescued;
    row.easyDamaged = row.isEasyHit && row.isDamaged;
    row.fdNegativeDamaged = row.isFdNegative && row.isDamaged;
    row.wideObjGainFromDefault = localTableScalar(wideRow, 'candidateObjGainFromDefault', NaN);
    row.defaultNonRefCoherenceFloor = localTableScalar(defaultRow, 'candidateNonRefCoherenceFloor', NaN);
    row.wideNonRefCoherenceFloor = localTableScalar(wideRow, 'candidateNonRefCoherenceFloor', NaN);
    row.wideDoaStep1Deg = localTableScalar(wideRow, 'candidateDoaStep1Deg', NaN);
    row.wideDoaStep2Deg = localTableScalar(wideRow, 'candidateDoaStep2Deg', NaN);
    wideDoaStepAbs = abs([row.wideDoaStep1Deg, row.wideDoaStep2Deg]);
    wideDoaStepAbs = wideDoaStepAbs(isfinite(wideDoaStepAbs));
    if isempty(wideDoaStepAbs)
      row.wideMaxAbsDoaStepDeg = NaN;
      row.wideDoaStepNormDeg = NaN;
    else
      row.wideMaxAbsDoaStepDeg = max(wideDoaStepAbs);
      row.wideDoaStepNormDeg = norm([row.wideDoaStep1Deg, row.wideDoaStep2Deg]);
    end
    row.wideFdRefStepHz = localTableScalar(wideRow, 'candidateFdRefStepHz', NaN);
    row.wideFdRateStepHzPerSec = localTableScalar(wideRow, 'candidateFdRateErrHzPerSec', NaN) - localTableScalar(defaultRow, 'candidateFdRateErrHzPerSec', NaN);
    rowCount = rowCount + 1;
    rowCell{rowCount} = row;
  end
end
if rowCount == 0
  ...
  decisionTable = struct2table([rowCell{1:rowCount}].');
end
end

function policySpec = localBuildWideGatePolicySpec(config)
%LOCALBUILDWIDEGATEPOLICYSPEC Build compact truth-free wide adoption policies.

objList = reshape(double(localGetFieldOrDefault(config, 'wideGateObjGainThresholdList', [0; 20; 50; 100])), [], 1);
minDoaStepList = reshape(double(localGetFieldOrDefault(config, 'wideGateMinAbsDoaStepDegList', [0])), [], 1);
maxDoaStepList = reshape(double(localGetFieldOrDefault(config, 'wideGateMaxAbsDoaStepDegList', [0.004])), [], 1);
rowCell = cell(1 + numel(objList) * numel(minDoaStepList) * numel(maxDoaStepList), 1);
rowCount = 1;
rowCell{rowCount} = localMakeWideGatePolicy("disabled", "disabled", Inf, 0, 0, false, config);
for iObj = 1:numel(objList)
  for iMinDoa = 1:numel(minDoaStepList)
    for iMaxDoa = 1:numel(maxDoaStepList)
      if minDoaStepList(iMinDoa) > maxDoaStepList(iMaxDoa)
        continue;
      end
      rowCount = rowCount + 1;
      label = sprintf('wide-obj%.4g-minStep%.4g-maxStep%.4g', objList(iObj), minDoaStepList(iMinDoa), maxDoaStepList(iMaxDoa));
      rowCell{rowCount} = localMakeWideGatePolicy(label, "wide-safe", objList(iObj), ...
        minDoaStepList(iMinDoa), maxDoaStepList(iMaxDoa), ...
        localGetFieldOrDefault(config, 'wideGateRequireDefaultNonCollapse', true), config);
    end
  end
end
policySpec = [rowCell{1:rowCount}];
end

function policy = localMakeWideGatePolicy(label, family, minObjGain, minAbsDoaStepDeg, maxAbsDoaStepDeg, requireDefaultNonCollapse, config)
%LOCALMAKEWIDEGATEPOLICY Pack one wide-gate policy descriptor.

policy = struct();
policy.label = string(label);
policy.family = string(family);
policy.minObjGain = minObjGain;
policy.minAbsDoaStepDeg = minAbsDoaStepDeg;
policy.maxAbsDoaStepDeg = maxAbsDoaStepDeg;
policy.minCandidateCoherence = localGetFieldOrDefault(config, 'wideGateCandidateCoherenceThreshold', 0.95);
policy.maxAbsFdRefStepHz = localGetFieldOrDefault(config, 'safeAdoptMaxAbsFdRefStepHz', 300);
policy.maxAbsFdRateStepHzPerSec = localGetFieldOrDefault(config, 'wideGateMaxAbsFdRateStepHzPerSec', Inf);
policy.requireDefaultNonCollapse = logical(requireDefaultNonCollapse);
policy.defaultNonCollapseThreshold = localGetFieldOrDefault(config, 'ordinaryCoherenceMinThreshold', 0.95);
end

function [useWide, gateReason] = localEvaluateWideGatePolicy(defaultRow, wideRow, policy, config)
%LOCALEVALUATEWIDEGATEPOLICY Apply one truth-free ordinary-wide policy.

useWide = false;
gateReason = "accepted";
if policy.family == "disabled"
  gateReason = "disabled";
  return;
end
if isempty(wideRow) || height(wideRow) == 0
  gateReason = "wide-unavailable";
  return;
end
if policy.requireDefaultNonCollapse
  defaultCoh = localTableScalar(defaultRow, 'candidateNonRefCoherenceFloor', NaN);
  if ~(isfinite(defaultCoh) && defaultCoh >= policy.defaultNonCollapseThreshold)
    gateReason = "default-not-noncollapse";
    return;
  end
end
objGain = localTableScalar(wideRow, 'candidateObjGainFromDefault', NaN);
if ~(isfinite(objGain) && objGain >= policy.minObjGain)
  gateReason = "obj-gain-too-small";
  return;
end
wideCoh = localTableScalar(wideRow, 'candidateNonRefCoherenceFloor', NaN);
if ~(isfinite(wideCoh) && wideCoh >= policy.minCandidateCoherence)
  gateReason = "wide-coherence-not-recovered";
  return;
end
doaStep = [localTableScalar(wideRow, 'candidateDoaStep1Deg', NaN), localTableScalar(wideRow, 'candidateDoaStep2Deg', NaN)];
if any(~isfinite(doaStep)) || max(abs(doaStep)) > policy.maxAbsDoaStepDeg
  gateReason = "doa-step-too-large";
  return;
end
if max(abs(doaStep)) < policy.minAbsDoaStepDeg
  gateReason = "doa-disagreement-too-small";
  return;
end
fdRefStep = localTableScalar(wideRow, 'candidateFdRefStepHz', NaN);
if ~(isfinite(fdRefStep) && abs(fdRefStep) <= policy.maxAbsFdRefStepHz)
  gateReason = "fdref-step-too-large";
  return;
end
fdRateStep = localTableScalar(wideRow, 'candidateFdRateErrHzPerSec', NaN) - localTableScalar(defaultRow, 'candidateFdRateErrHzPerSec', NaN);
if isfinite(policy.maxAbsFdRateStepHzPerSec) && ~(isfinite(fdRateStep) && abs(fdRateStep) <= policy.maxAbsFdRateStepHzPerSec)
  gateReason = "fdrate-step-too-large";
  return;
end
useWide = true;
gateReason = "accepted";
...
function policyTable = localBuildWideGatePolicyTable(decisionTable, config)
%LOCALBUILDWIDEGATEPOLICYTABLE Summarize ordinary-wide gate policies.

if isempty(decisionTable)
  policyTable = table();
  return;
end
policyList = unique(decisionTable.policyLabel, 'stable');
disabledMask = decisionTable.policyFamily == "disabled";
disabledP95 = NaN;
disabledMax = NaN;
if any(disabledMask)
  disabledP95 = localPercentile(decisionTable.selectedAngleErrDeg(disabledMask), 95);
  disabledMax = max(decisionTable.selectedAngleErrDeg(disabledMask), [], 'omitnan');
end
rowCell = cell(numel(policyList), 1);
for iPolicy = 1:numel(policyList)
  policyNow = policyList(iPolicy);
  mask = decisionTable.policyLabel == policyNow;
  ordinaryMask = mask & decisionTable.isOrdinaryAngleMiss;
  easyMask = mask & decisionTable.isEasyHit;
  fdMask = mask & decisionTable.isFdNegative;
  row = struct();
  row.policyLabel = policyNow;
  row.policyFamily = decisionTable.policyFamily(find(mask, 1, 'first'));
  row.numSeed = nnz(mask);
  row.numOrdinaryMiss = nnz(ordinaryMask);
  row.minObjGain = decisionTable.minObjGain(find(mask, 1, 'first'));
  row.minAbsDoaStepDeg = decisionTable.minAbsDoaStepDeg(find(mask, 1, 'first'));
  row.maxAbsDoaStepDeg = decisionTable.maxAbsDoaStepDeg(find(mask, 1, 'first'));
  row.requireDefaultNonCollapse = decisionTable.requireDefaultNonCollapse(find(mask, 1, 'first'));
  row.triggerRate = mean(double(decisionTable.gateTriggered(mask)), 'omitnan');
  row.ordinaryTriggerRate = localMeanOptionalLogical(decisionTable.gateTriggered(ordinaryMask));
  row.easyTriggerRate = localMeanOptionalLogical(decisionTable.gateTriggered(easyMask));
  row.easyTriggerCount = nnz(decisionTable.gateTriggered(easyMask));
  row.ordinaryRescueRate = localMeanOptionalLogical(decisionTable.ordinaryRescued(ordinaryMask));
  row.ordinaryHitRate = localMeanOptionalLogical(decisionTable.isHit(ordinaryMask));
  row.easyDamageRate = localMeanOptionalLogical(decisionTable.easyDamaged(easyMask));
  row.fdNegativeDamageRate = localMeanOptionalLogical(decisionTable.fdNegativeDamaged(fdMask));
  row.overallHitRate = localMeanHit(decisionTable.selectedAngleErrDeg(mask), config.angleHitThresholdDeg);
  row.overallMedianAngleDeg = median(decisionTable.selectedAngleErrDeg(mask), 'omitnan');
  row.overallP95AngleDeg = localPercentile(decisionTable.selectedAngleErrDeg(mask), 95);
  row.overallP99AngleDeg = localPercentile(decisionTable.selectedAngleErrDeg(mask), 99);
  row.overallMaxAngleDeg = max(decisionTable.selectedAngleErrDeg(mask), [], 'omitnan');
  row.medianAngleGainDeg = median(decisionTable.angleGainDeg(mask), 'omitnan');
  row.p95NotWorseThanDisabled = row.overallP95AngleDeg <= disabledP95 + 1e-12;
  row.maxNotWorseThanDisabled = row.overallMaxAngleDeg <= disabledMax + 1e-12;
  row.isPreferredPreset = localIsWideGatePreferredPreset(row, config);
  row.isStrictCandidate = row.numOrdinaryMiss > 0 && row.ordinaryRescueRate >= 1 - 1e-12 && ...
    row.ordinaryHitRate >= 1 - 1e-12 && row.easyDamageRate == 0 && row.fdNegativeDamageRate == 0 && ...
    row.easyTriggerRate <= localGetFieldOrDefault(config, 'wideGateStrictEasyTriggerRateMax', 0.10) && ...
    row.p95NotWorseThanDisabled && row.maxNotWorseThanDisabled;
  row.recommendedPass = row.numOrdinaryMiss > 0 && row.ordinaryRescueRate >= 0.75 && ...
    row.easyDamageRate == 0 && row.fdNegativeDamageRate == 0 && row.p95NotWorseThanDisabled && row.maxNotWorseThanDisabled;
  if row.isStrictCandidate && row.isPreferredPreset
    row.recommendation = "preferred-candidate";
  elseif row.isStrictCandidate
    row.recommendation = "candidate";
  elseif row.recommendedPass && row.easyTriggerRate <= 0.25
    row.recommendation = "candidate-partial";
  elseif row.recommendedPass
    row.recommendation = "candidate-wide-trigger";
  elseif row.ordinaryRescueRate > 0 && (row.easyDamageRate > 0 || row.fdNegativeDamageRate > 0)
    row.recommendation = "unsafe";
  else
    row.recommendation = "hold";
  end
  rowCell{iPolicy} = row;
end
policyTable = struct2table([rowCell{:}].');
end

function selectedPolicyTable = localSelectWideGateRecommendedPolicy(policyTable, config)
%LOCALSELECTWIDEGATERECOMMENDEDPOLICY Pick the most useful ordinary-wide replay candidate.

if isempty(policyTable)
  selectedPolicyTable = table();
  return;
end
nonDisabledMask = policyTable.policyFamily ~= "disabled";
if any(policyTable.isStrictCandidate)
  candidateMask = policyTable.isStrictCandidate;
elseif any(policyTable.recommendedPass & nonDisabledMask)
  candidateMask = policyTable.recommendedPass & nonDisabledMask;
elseif any(nonDisabledMask)
  candidateMask = nonDisabledMask;
else
  candidateMask = true(height(policyTable), 1);
end
idxList = find(candidateMask);
score = localWideGatePolicyScore(policyTable(idxList, :), config);
[~, bestRelIdx] = max(score);
selectedPolicyTable = policyTable(idxList(bestRelIdx), :);
selectedPolicyTable.selectionScore = score(bestRelIdx);
end

function score = localWideGatePolicyScore(policyTable, config)
%LOCALWIDEGATEPOLICYSCORE Rank policies by rescue quality, damage risk, and trigger narrowness.

n = height(policyTable);
score = zeros(n, 1);
for iRow = 1:n
  score(iRow) = 1000 * double(policyTable.isStrictCandidate(iRow)) + ...
    300 * double(policyTable.isPreferredPreset(iRow)) + ...
    100 * localSafeMetric(policyTable.ordinaryRescueRate(iRow), -1) + ...
    60 * localSafeMetric(policyTable.ordinaryHitRate(iRow), -1) + ...
    20 * localSafeMetric(policyTable.overallHitRate(iRow), -1) - ...
    120 * localSafeMetric(policyTable.easyDamageRate(iRow), 1) - ...
    120 * localSafeMetric(policyTable.fdNegativeDamageRate(iRow), 1) - ...
    20 * localSafeMetric(policyTable.easyTriggerRate(iRow), 1) - ...
    4 * localSafeMetric(policyTable.triggerRate(iRow), 1) - ...
    2 * localSafeMetric(policyTable.overallP95AngleDeg(iRow), 1) - ...
    localSafeMetric(policyTable.overallMaxAngleDeg(iRow), 1);
  if localIsWideGatePreferredPreset(policyTable(iRow, :), config)
    score(iRow) = score(iRow) + 10;
  end
end
end

function tf = localIsWideGatePreferredPreset(row, config)
%LOCALISWIDEGATEPREFERREDPRESET Check whether a policy matches the current replay-level preferred gate.

tol = 1e-12;
tf = row.policyFamily == "wide-safe" && ...
  abs(row.minObjGain - localGetFieldOrDefault(config, 'wideGatePreferredObjGain', 10)) <= tol && ...
  abs(row.minAbsDoaStepDeg - localGetFieldOrDefault(config, 'wideGatePreferredMinAbsDoaStepDeg', 0.001)) <= tol && ...
  abs(row.maxAbsDoaStepDeg - localGetFieldOrDefault(config, 'wideGatePreferredMaxAbsDoaStepDeg', 0.004)) <= tol;
end

function value = localSafeMetric(value, nanFallback)
%LOCALSAFEMETRIC Replace nonfinite ranking metrics with a deterministic fallback.
...
function value = localTableScalar(tableUse, fieldName, defaultValue)
%LOCALTABlESCALAR Return one scalar table value with fallback.
...
function localPlotWideGateReplay(plotData)
%LOCALPLOTWIDEGATEREPLAY Draw compact wide-gate policy diagnostics.

if isempty(plotData) || ~isstruct(plotData)
  return;
end
policyTable = localGetFieldOrDefault(plotData, 'wideGatePolicyTable', table());
decisionTable = localGetFieldOrDefault(plotData, 'wideGateDecisionTable', table());
if isempty(policyTable)
  return;
end
figure('Name', 'Ordinary-wide gate diagnose', 'Color', 'w');
tiledlayout(3, 1, 'TileSpacing', 'compact');

nexttile;
x = 1:height(policyTable);
plot(x, policyTable.ordinaryRescueRate, '-o');
hold on;
plot(x, policyTable.easyDamageRate, '-s');
plot(x, policyTable.fdNegativeDamageRate, '-^');
ylabel('rate');
title('Wide-gate rescue / damage policy sweep');
grid on;
legend(["ordinary rescue", "easy damage", "fd-negative damage"], 'Location', 'best');
xticks(x);
xticklabels(policyTable.policyLabel);
xtickangle(35);

nexttile;
plot(x, policyTable.overallP95AngleDeg, '-o');
hold on;
plot(x, policyTable.overallMaxAngleDeg, '-s');
ylabel('angle error (deg)');
title('Overall tail after policy adoption');
grid on;
...
nexttile;
if ~isempty(decisionTable)
  accepted = decisionTable(decisionTable.gateTriggered, :);
  if ~isempty(accepted)
    scatter(accepted.wideObjGainFromDefault, accepted.angleGainDeg, 28, 'filled');
    xlabel('wide objective gain from default');
    ylabel('angle gain (deg)');
    title('Accepted wide candidates across policies');
    grid on;
  else
    text(0.1, 0.5, 'No accepted wide candidates', 'Units', 'normalized');
    axis off;
  end
else
  axis off;
...
if ~(isfinite(toothStepHz) && toothStepHz > 0)
  error('replayMfInToothOrdinaryWideGateDiagnose:MissingToothStep', ...
...
metricLineList = [metricLineList; localBuildTelegramOrdinaryLines(replayData)];
metricLineList = [metricLineList; localBuildTelegramWideGateLines(replayData)];
...
if isfield(replayData, 'ordinaryFamilyAggregateTable') && height(replayData.ordinaryFamilyAggregateTable) > 0
  ...
  lineList(end + 1, 1) = sprintf('• best family: <code>%s</code>, median=<code>%.4g deg</code>, rescue=<code>%.3f</code>', ...
    localHtmlEscape(t.candidateGroup(idx)), t.medianAngleErrDeg(idx), t.rescueRate(idx));
end
end

function lineList = localBuildTelegramWideGateLines(replayData)
%LOCALBUILDTELEGRAMWIDEGATELINES Extract wide-gate metrics for Telegram.

lineList = strings(0, 1);
if isfield(replayData, 'wideGateRecommendedPolicyTable') && height(replayData.wideGateRecommendedPolicyTable) > 0
  t = replayData.wideGateRecommendedPolicyTable;
elseif isfield(replayData, 'wideGatePolicyTable') && height(replayData.wideGatePolicyTable) > 0
  t = localSelectWideGateRecommendedPolicy(replayData.wideGatePolicyTable, replayData.config);
else
  return;
end
lineList(end + 1, 1) = sprintf(['• wide gate: selected=<code>%s</code>, ordinary rescue=<code>%.3f</code>, ' ...
  'easy trig/damage=<code>%.3f/%.3f</code>, fd damage=<code>%.3f</code>, rec=<code>%s</code>'], ...
  localHtmlEscape(t.policyLabel(1)), t.ordinaryRescueRate(1), t.easyTriggerRate(1), ...
  t.easyDamageRate(1), t.fdNegativeDamageRate(1), localHtmlEscape(t.recommendation(1)));
end
