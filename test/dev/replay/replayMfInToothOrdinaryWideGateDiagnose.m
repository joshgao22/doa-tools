...
safeSuffix = sprintf('ordinaryCoh%.3f_hit%.4f_finalPolish%d_singleMf%d_hardAux%d_familyStep%.3f_joint%d_wideGateObj%d_minStep%d_maxStep%d_coh%.3f_noncol%d', ...
  config.ordinaryCoherenceMinThreshold, config.angleHitThresholdDeg, ...
  config.includeFinalSmallPolishProbe, config.includeSingleMfBasinEntryProbe, ...
  config.includeHardCollapseAuxTables, config.familySafeAdoptMaxAbsDoaStepDeg, ...
  config.includeJointSafeAdoptBank, numel(config.wideGateObjGainThresholdList), ...
  numel(config.wideGateMinAbsDoaStepDegList), numel(config.wideGateMaxAbsDoaStepDegList), ...
  config.wideGateCandidateCoherenceThreshold, config.wideGateRequireDefaultNonCollapse);
