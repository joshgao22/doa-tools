% Diagnostic compare for the 1/T_f periodic-branch hypothesis.
% This script keeps the current MS-MF-CP pipeline but compares:
%   1) the default wide search ranges;
%   2) a truth-assisted narrow [fdRef, fdRate] box;
%   3) a stronger truth-assisted narrow box with truth-centered DoA.
% The goal is not to change the main algorithm, but to verify whether the
% wrong branch disappears once the true Doppler tooth is isolated.
clear(); close all; clc;

localAddProjectPath();
fixture = localBuildPipelineFixture(-4:5);

toothStepHz = 1 / fixture.frameIntvlSec;
truthFdHalfWidthHz = 200;
truthFdRateHalfWidthHzPerSec = 500;

fprintf('Running compareMfPeriodicTruthNarrowBox ...\n');

baselineCase = localRunMsUnknownCase(fixture, fixture.fdRange, fixture.fdRateRange, struct(), struct());

fdRangeTruth = [fixture.truth.fdRefFit - truthFdHalfWidthHz, ...
  fixture.truth.fdRefFit + truthFdHalfWidthHz];
fdRateRangeTruth = [fixture.truth.fdRateFit - truthFdRateHalfWidthHzPerSec, ...
  fixture.truth.fdRateFit + truthFdRateHalfWidthHzPerSec];

freqPriorCase = localRunMsUnknownCase(fixture, fdRangeTruth, fdRateRangeTruth, struct(), struct());

doaTruthOpt = struct();
doaTruthOpt.initDoaParam = fixture.truth.latlonTrueDeg(:);
doaTruthOpt.initDoaHalfWidth = [0.01; 0.01];
fullPriorCase = localRunMsUnknownCase(fixture, fdRangeTruth, fdRateRangeTruth, doaTruthOpt, doaTruthOpt);

baselineSummary = localBuildCaseSummary(baselineCase, fixture.truth, toothStepHz);
freqPriorSummary = localBuildCaseSummary(freqPriorCase, fixture.truth, toothStepHz);
fullPriorSummary = localBuildCaseSummary(fullPriorCase, fixture.truth, toothStepHz);

if abs(freqPriorSummary.toothIdx) > 0
  error('compareMfPeriodicTruthNarrowBox:WrongToothUnderTruthFdPrior', ...
    'The truth-assisted narrow fdRef box should exclude the adjacent 1/T_f teeth.');
end
if abs(fullPriorSummary.toothIdx) > 0
  error('compareMfPeriodicTruthNarrowBox:WrongToothUnderTruthDoaFdPrior', ...
    'The stronger truth-assisted prior should stay on the truth Doppler tooth.');
end
if ~baselineSummary.isResolved || ~freqPriorSummary.isResolved || ~fullPriorSummary.isResolved
  error('compareMfPeriodicTruthNarrowBox:UnresolvedCase', ...
    'All comparison runs should resolve in this regression script.');
end

fprintf('  tooth step (Hz)                  : %.6f\n', toothStepHz);
fprintf('  truth-narrow fdRange             : %s\n', localFormatNumericRow(fdRangeTruth));
fprintf('  truth-narrow fdRateRange         : %s\n', localFormatNumericRow(fdRateRangeTruth));
localPrintCaseSummary('baseline wide', baselineSummary);
localPrintCaseSummary('truth-narrow fd/fdRate', freqPriorSummary);
localPrintCaseSummary('truth-narrow fd/fdRate + DoA', fullPriorSummary);
fprintf('PASS: compareMfPeriodicTruthNarrowBox\n');
...
if isempty(refFrameIdx)
  error('compareMfPeriodicTruthNarrowBox:MissingReferenceFrame', ...
...
elseif ndims(localDoaRaw) == 4
  ...
  error('compareMfPeriodicTruthNarrowBox:InvalidLocalDoaSize', ...
