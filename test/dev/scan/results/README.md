...
| `scanMfMsMleCrbCleanBoundConsistency.m` | `scanMfMsMleCrbCleanBoundConsistency.md` | `paper-facing / controlled-local / representative` | 20260516 full-SNR 代表性结果固定 `(DoA, fdRef) = (2, 2.5)` CRB-scale；`MS-MF-CP-K/U` 在 joint-trim 口径下达到 CRB-level，MS 显著降低绝对 DoA 误差，CP 相比 IP 保留数量级更强的连续相位信息量；必须标注 truth-centered local-bound 条件。 |
| `scanMfMsMleCrbCleanBoundScale.m` | `scanMfMsMleCrbCleanBoundScale.md` | `diagnostic / controlled-local range surface / representative` | 0 dB、30 repeats、6 星 CleanBoundScale 细化网格支持后续全 SNR 主 bound 取 `(DoA, fdRef) = (2, 2.5)`；`(2.5, 2.25)` 可作 sensitivity；不作为 full-flow acquisition 证明。 |
| `scanMfGeoSatGeometryCrbAudit.m` | `scanMfGeoSatGeometryCrbAudit.md` | `paper-facing scenario screening / coarse` | Geo-level Starlink-inspired location + satellite-set coarse screen；当前代表性结果将 `[45,36.59,0]` 标为 L=4 main candidate，将 `[60,36.59,0]` 降为 stress candidate，并把原 `[37.78,36.59,0]` 降级为 anchor / stress 对照。 |
| `scanMfPairGeometryCrbAudit.m` | `scanMfPairGeometryCrbAudit.md` | `paper-facing scenario screening / representative` | 固定位置 satellite-set 深挖；当前代表性结果推荐 `[55,36.59,0]`、`refSat=5259`、`L=6 [5259 1243 348 5652 14 4437]` 作为论文主仿真组合，`L=4` 作轻量 smoke，`L=8` 作增强确认；`[45,36.59,0]` 系列和纯 CRB 最小高星数组合更适合作 hard-coupled stress 对照。 |
| `scanMfMsMleCrbInToothConsistency.m` | `scanMfMsMleCrbInToothConsistency.md` | `diagnostic-only / replay-seed index` | MS-only in-tooth scan 的 1000-repeat 错误类型统计；固定 `MS-MF-CP-K/U` 在各 SNR 下的 `fdref-branch-tail`、`fd-rate-tail`、`doa-basin-limited`、`coherence-collapse-tail` 分布和代表 seed，用于指导 `replayMfMsMleCrbFlowDiagnose` 定点排查。 |
