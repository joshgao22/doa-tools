% Diagnostic scan of DoA-tooth coupling in the MF objective.
% This script evaluates the dynamic profile objective on a 2D slice:
%   1) DoA points along the line from truth to the periodic wide-solve point;
%   2) fdRef teeth k / T_f with k in {-1, 0, 1}.
% The fdRate profile is kept at truth so the slice focuses on how DoA and
% tooth selection couple inside the current CP evaluator.
clear(); close all; clc;

localAddProjectPath();

periodicOffsetIdx = -4:5;
nonPeriodicOffsetIdx = [-9, -7, -6, -5, -4, 0, 1, 3, 7, 8];
...
baselinePeriodic = localRunMsUnknownCase(fixturePeriodic, fixturePeriodic.fdRange, fixturePeriodic.fdRateRange);
truthLatlon = fixturePeriodic.truth.latlonTrueDeg(:);
badLatlon = baselinePeriodic.estResult.doaParamEst(:);
lineDir = badLatlon - truthLatlon;
if norm(lineDir) < 1e-8
  lineDir = [1; 0] * 0.02;
else
  lineDir = lineDir / norm(lineDir) * max(norm(badLatlon - truthLatlon), 0.02);
end
lineScaleList = [-0.50, -0.25, 0, 0.25, 0.50, 0.75, 1.00, 1.25];
toothIdxList = -1:1;

doaMat = truthLatlon + lineDir .* reshape(lineScaleList, 1, []);
curvePeriodic = localEvalDoaToothSurface(fixturePeriodic, doaMat, toothIdxList, toothStepHz);
curveNonPeriodic = localEvalDoaToothSurface(fixtureNonPeriodic, doaMat, toothIdxList, toothStepHz);

[bestToothPeriodic, bestToothIdxPeriodic] = min(curvePeriodic.objMat, [], 1);
[bestToothNonPeriodic, bestToothIdxNonPeriodic] = min(curveNonPeriodic.objMat, [], 1);
truthToothPos = find(toothIdxList == 0, 1);
minOffGapPeriodic = min(curvePeriodic.objMat(setdiff(1:numel(toothIdxList), truthToothPos), :) - curvePeriodic.objMat(truthToothPos, :), [], 1);
minOffGapNonPeriodic = min(curveNonPeriodic.objMat(setdiff(1:numel(toothIdxList), truthToothPos), :) - curveNonPeriodic.objMat(truthToothPos, :), [], 1);

if ~all(isfinite(curvePeriodic.objMat(:))) || ~all(isfinite(curveNonPeriodic.objMat(:)))
  error('scanMfDoaToothSlice:InvalidObjective', ...
    'All DoA-tooth slice objective values must stay finite.');
end

fprintf('Running scanMfDoaToothSlice ...\n');
fprintf('  tooth step (Hz)                    : %.6f\n', toothStepHz);
fprintf('  tooth index list                   : %s\n', localFormatIntegerRow(toothIdxList));
fprintf('  line scales                        : %s\n', localFormatNumericRow(lineScaleList));
fprintf('  truth lat/lon (deg)                : %s\n', localFormatNumericRow(truthLatlon));
fprintf('  periodic wide lat/lon (deg)        : %s\n', localFormatNumericRow(badLatlon));
fprintf('  periodic objective matrix          :\n');
disp(array2table(curvePeriodic.objMat, 'VariableNames', localBuildDoaColumnNames(doaMat), 'RowNames', localBuildToothRowNames(toothIdxList)));
fprintf('  non-periodic objective matrix      :\n');
disp(array2table(curveNonPeriodic.objMat, 'VariableNames', localBuildDoaColumnNames(doaMat), 'RowNames', localBuildToothRowNames(toothIdxList)));
fprintf('  periodic best tooth by DoA         : %s\n', localFormatIntegerRow(toothIdxList(bestToothIdxPeriodic)));
fprintf('  non-periodic best tooth by DoA     : %s\n', localFormatIntegerRow(toothIdxList(bestToothIdxNonPeriodic)));
fprintf('  periodic min off-truth gap by DoA  : %s\n', localFormatNumericRow(minOffGapPeriodic));
fprintf('  non-periodic min off-truth gap by DoA : %s\n', localFormatNumericRow(minOffGapNonPeriodic));
if any(toothIdxList(bestToothIdxPeriodic) ~= 0)
  fprintf('  note                               : periodic slice still prefers off-truth teeth for some DoA points.\n');
end
if all(toothIdxList(bestToothIdxNonPeriodic) == 0)
  fprintf('  note                               : non-periodic slice keeps the truth tooth across the whole DoA line.\n');
end
fprintf('PASS: scanMfDoaToothSlice\n');


function curve = localEvalDoaToothSurface(fixture, doaMat, toothIdxList, toothStepHz)
modelOpt = struct();
...
numDoa = size(doaMat, 2);
numTooth = numel(toothIdxList);
objMat = nan(numTooth, numDoa);

parfor iDoa = 1:numDoa
  localDoaArrUse = localBuildLocalDoaArrFromLatlon(doaMat(:, iDoa), fixture.sceneSeq);
  objCol = nan(numTooth, 1);
  for iTooth = 1:numTooth
    hypUse = struct();
    hypUse.localDoaArr = localDoaArrUse;
    hypUse.fdSat = fixture.truth.fdSatFitHz(:) + toothIdxList(iTooth) * toothStepHz;
    hypUse.fdRateSat = fixture.truth.fdRateSatTrueHzPerSec(:);
    [objCol(iTooth), ~] = evalDoaDopplerDynProfileLike(modelUse, hypUse);
  end
  objMat(:, iDoa) = objCol;
end
...
function localDoaArr = localBuildLocalDoaArrFromLatlon(latlonDeg, sceneSeq)
[userPosSeq, ~] = buildUserStateFromLatlon(latlonDeg(:), sceneSeq);
localDoaArr = nan(2, sceneSeq.numSat, sceneSeq.numFrame);
for iFrame = 1:sceneSeq.numFrame
  satPosMat = sceneSeq.satPosEci(:, :, iFrame);
  userPosUse = reshape(userPosSeq(:, iFrame), 3, 1);
  for iSat = 1:sceneSeq.numSat
    rotMatUse = sceneSeq.rotMat{iFrame, iSat};
    localDoaArr(:, iSat, iFrame) = globalToLocalDoa(userPosUse, satPosMat(:, iSat), rotMatUse);
  end
end
end
...
if isempty(refFrameIdx)
  error('scanMfDoaToothSlice:MissingReferenceFrame', ...
...
elseif ndims(localDoaRaw) == 4
  ...
  error('scanMfDoaToothSlice:InvalidLocalDoaSize', ...
...
function nameCell = localBuildDoaColumnNames(doaMat)
nameCell = cell(1, size(doaMat, 2));
for iCol = 1:size(doaMat, 2)
  nameCell{iCol} = sprintf('lat%.4f_lon%.4f', doaMat(1, iCol), doaMat(2, iCol));
end
end


function nameCell = localBuildToothRowNames(toothIdxList)
nameCell = cell(1, numel(toothIdxList));
for iRow = 1:numel(toothIdxList)
  nameCell{iRow} = sprintf('k_%+d', toothIdxList(iRow));
end
end
