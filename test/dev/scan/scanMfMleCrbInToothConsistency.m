%SCANMFMLECRBINTOOTHCONSISTENCY Scan in-tooth MLE consistency with CRB.
...
scanName = "scanMfMleCrbInToothConsistency";
saveSnapshot = true;
...
try
  ...
  notifyMfScanStatus(struct( ...
    ...
    'commentLineList', [ ...
      "In-tooth MLE-vs-CRB local consistency scan completed."; ...
      "Use resolved-sample RMSE/CRB with full-sample outlier accounting, not angle hit rate alone."]));
...
if ~any(string(initMode) == validModeList)
  error('scanMfMleCrbInToothConsistency:InvalidInitMode', ...
...
if numFrame < 2
  error('scanMfMleCrbInToothConsistency:InvalidFrameCount', ...
...
if isempty(requestedNameList)
  error('scanMfMleCrbInToothConsistency:EmptyMethodNameList', ...
...
if numel(unique(requestedNameList, 'stable')) ~= numel(requestedNameList)
  error('scanMfMleCrbInToothConsistency:DuplicateMethodName', ...
...
if any(~isKnownMethod)
  error('scanMfMleCrbInToothConsistency:UnknownMethodName', ...
...
if ~(isfinite(truthFdRefHz) && isfinite(truthFdRateHzPerSec))
  error('scanMfMleCrbInToothConsistency:MissingTruthFdLine', ...
...
switch method.viewMode
  ...
  otherwise
    error('scanMfMleCrbInToothConsistency:UnknownViewMode', ...
...
for iFrame = 1:numel(frameList)
  ...
  grid on;
  xlabel('SNR (dB)');
  ylabel('Global angle RMSE / CRB (deg)');
