...
actualStartTagList = reshape(string(multiStartDiag.startTagList), [], 1);
requiredTag = ["fromCpK"; "fromStatic"];
if ~all(ismember(requiredTag, actualStartTagList))
...
expectedIdx = localSelectBestDynamicCandidateContract( ...
  multiStartDiag.fvalList, multiStartDiag.isResolvedList, multiStartDiag.startTagList, ...
  multiStartDiag.iterationList, multiStartDiag.objImproveList, multiStartDiag.moveNormList);
expectedStartTag = string(multiStartDiag.startTagList(expectedIdx));
actualIdx = multiStartDiag.selectedIdx;
actualStartTag = string(multiStartDiag.selectedStartTag);
...
summarySelectedStartTag = string(summaryTable.startTag(summarySelectedMask));

if actualIdx ~= expectedIdx || actualStartTag ~= expectedStartTag
  error('regressionMfUnknownBestStartSelection:SelectedCandidateMismatch', ...
    ['runDynamicDoaDopplerCase selected %s (idx %d), but the documented ', ...
     'selection rule prefers %s (idx %d).'], actualStartTag, actualIdx, expectedStartTag, expectedIdx);
end
if summarySelectedStartTag ~= actualStartTag
  error('regressionMfUnknownBestStartSelection:SummaryTagMismatch', ...
    ...
    summarySelectedStartTag, actualStartTag);
end

fprintf('  candidate tags           : %s\n', localFormatStringRow(actualStartTagList));
...
startTagList = reshape(string(startTagList), [], 1);
...
preferStatic = contains(lower(startTagList(nearBest)), 'static');
