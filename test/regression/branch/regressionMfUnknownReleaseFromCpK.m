...
fdRefErrHz = abs(solveUnknown.finalEvalDiag.fdRef - fixture.truth.fdRefFit);
angleErrDeg = calcLatlonAngleError(reshape(solveUnknown.finalEvalDiag.latlon, 2, []), ...
  fixture.truth.latlonTrueDeg(:));

if moveNorm <= 1e-6
  error('regressionMfUnknownReleaseFromCpK:NoReleaseMove', ...
    'CP-U did not move away from the CP-K seed after release.');
end
if fdRateMoveHzPerSec <= 1e-3
  error('regressionMfUnknownReleaseFromCpK:FdRateDidNotRelease', ...
    'CP-U fdRate stayed numerically locked to the CP-K seed.');
end
if solveUnknown.fval > solveKnown.fval + objTol
  error('regressionMfUnknownReleaseFromCpK:UnknownWorseThanSeed', ...
    'Released CP-U fit is worse than the biased CP-K seed.');
end
if finalTruthGap >= seedTruthGap - 1e-6
  error('regressionMfUnknownReleaseFromCpK:FdRateNotCloserToTruth', ...
    'Released CP-U fdRate did not move closer to truth.');
end
if ~isfield(optimInfoUnknown, 'candidateVariant') || isempty(optimInfoUnknown.candidateVariant)
  error('regressionMfUnknownReleaseFromCpK:MissingUnknownCandidates', ...
    'Released CP-U solve must expose continuation candidates.');
end

fprintf('  CP-K seed solveVariant      : %s\n', string(optimInfoKnown.solveVariant));
fprintf('  CP-U final solveVariant     : %s\n', string(optimInfoUnknown.solveVariant));
...
fprintf('  CP-K seed fval             : %.6e\n', solveKnown.fval);
fprintf('  CP-U final fval            : %.6e\n', solveUnknown.fval);
fprintf('  move norm                   : %.6f\n', moveNorm);
