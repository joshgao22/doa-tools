...
allowedTagMask = strcmp(candidateVariant, "main") | strcmp(candidateVariant, "mainWarmAnchor") | contains(candidateVariant, "unknownWarmLocal_fdRate");
if ~all(allowedTagMask)
  error('regressionMfBranchKnownUnknown:InvalidUnknownCandidateTag', ...
    'CP-U continuation candidates expose unexpected solveVariant tags after the branch simplification.');
end
if ~any(strcmp(candidateVariant, "mainWarmAnchor")) && ~any(contains(candidateVariant, "unknownWarmLocal_fdRate"))
  error('regressionMfBranchKnownUnknown:MissingContinuationAnchor', ...
    'CP-U continuation candidates must include either mainWarmAnchor or a legacy unknownWarmLocal anchor.');
end
