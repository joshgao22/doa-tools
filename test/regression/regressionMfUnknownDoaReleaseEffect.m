% Regression check for CP-U DoA-release effect.
...
fprintf('Running regressionMfUnknownDoaReleaseEffect ...\n');
...
if doaMoveFdOnlyDeg > 1e-5
  error('regressionMfUnknownDoaReleaseEffect:FdOnlyDoaMoved', ...
...
if ~isfield(optimInfoUnknownFdOnly, 'candidateVariant') || isempty(optimInfoUnknownFdOnly.candidateVariant) || ...
  ...
  error('regressionMfUnknownDoaReleaseEffect:MissingUnknownCandidates', ...
...
if doaMoveDoaDeg <= doaMoveFdOnlyDeg + 1e-6 && angleImproveDeg <= 1e-6 && ...
  ...
  error('regressionMfUnknownDoaReleaseEffect:NoDoaReleaseEffect', ...
...
fprintf('PASS: regressionMfUnknownDoaReleaseEffect\n');
