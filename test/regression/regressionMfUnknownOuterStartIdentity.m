% Regression check for CP-U outer warm-start identity.
...
fprintf('Running regressionMfUnknownOuterStartIdentity ...\n');
...
if isempty(initParamStaticMs)
  error('regressionMfUnknownOuterStartIdentity:MissingStaticInit', ...
...
if srcCpKAngleErrDeg > 1e-9 || srcStaticAngleErrDeg > 1e-9 || ...
  ...
  error('regressionMfUnknownOuterStartIdentity:SourceMismatch', ...
...
if packedGap.paramGapNorm <= collapseTol && packedGap.doaGapDeg <= 1e-9 && ...
  ...
  error('regressionMfUnknownOuterStartIdentity:OuterStartsCollapsed', ...
    ['The fromCpK and fromStatic outer warm starts collapsed before CP-U ', ...
     'solving. Inferred stage: %s.'], traceStage);
end

fprintf('PASS: regressionMfUnknownOuterStartIdentity\n');
...
if isstruct(candObj)
  ...
  if isfield(candObj, 'tagList')
    srcTag = reshape(string(candObj.tagList), 1, []);
  elseif isfield(candObj, 'tag')
    srcTag = reshape(string(candObj.tag), 1, []);
  else
    srcTag = strings(1, 0);
  end
